...
test('owner previews and archives QA records through a verified maintenance decision', async ({ page, request }) => {
  ...
  await expect(counts.locator('div').filter({ hasText: 'Jobs' })).toContainText('1')
  await expect(counts.locator('div').filter({ hasText: 'Opportunities' })).toContainText('1')
  await expect(counts.locator('div').filter({ hasText: 'Workers' })).toContainText('1')
  await expect(counts.locator('div').filter({ hasText: 'Equipment' })).toContainText('1')

  let submit = dialog.getByRole('button', { name: 'Archive 4 record(s)' })
  ...
  await dialog.getByLabel('Maintenance reason').fill('Remove verified browser fixtures from active release queues.')
  await dialog.getByLabel('Type ARCHIVE QA to confirm').fill('ARCHIVE QA')
  submit = dialog.getByRole('button', { name: 'Archive 4 record(s)' })
  await submit.click()
  await expect(dialog).toBeHidden()
  await expect(opener).toBeFocused()
