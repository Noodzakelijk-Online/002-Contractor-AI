...
test('backup verification rejects a migration 069 ledger with missing framework revision constraints', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-framework-schema-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Framework schema fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec('DROP INDEX idx_framework_revisions_implementation');
  database.close();
  assert.throws(
...
test('backup verification rejects a migration 071 ledger with missing data-subject request constraints', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-privacy-request-schema-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Privacy request schema fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec('DROP INDEX idx_data_subject_requests_status_due');
  database.close();
  assert.throws(
    () => verifySqliteBackupDatabase(ledgerFile),
    /data-subject request constraints are incomplete: idx_data_subject_requests_status_due/i
  );
});

test('backup verification rejects a migration 072 ledger with missing operator preference constraints', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-locale-schema-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Locale preference schema fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec('DROP INDEX idx_operator_preferences_locale');
  database.close();
  assert.throws(
    () => verifySqliteBackupDatabase(ledgerFile),
    /operator-preference constraints are incomplete: idx_operator_preferences_locale/i
  );
});
...
test('backup verification accepts a complete migration 062 ledger without migration 063', t => {
  ...
  database.exec(`
    ...
    DROP TABLE installation_qc_controls;
    DROP TABLE lmra_assessments;
    DELETE FROM ledger_schema_migrations WHERE version IN ('063_governed_lmra', '064_governed_installation_qc', '065_governed_photo_evidence', '066_governed_client_feedback', '067_governed_energy_performance', '068_operational_safety_controls', '069_governed_framework_workspace', '070_managed_operator_accounts', '071_data_subject_request_governance', '072_operator_locale_preferences');
  `);
  database.close();
  assert.equal(verifySqliteBackupDatabase(ledgerFile).valid, true);
});
...
test('backup verification accepts a complete migration 063 ledger without migration 064', t => {
  ...
  database.exec(`
    ...
    DROP TABLE photo_evidence_sets;
    DROP TABLE installation_qc_controls;
    DELETE FROM ledger_schema_migrations WHERE version IN ('064_governed_installation_qc', '065_governed_photo_evidence', '066_governed_client_feedback', '067_governed_energy_performance', '068_operational_safety_controls', '069_governed_framework_workspace', '070_managed_operator_accounts', '071_data_subject_request_governance', '072_operator_locale_preferences');
  `);
  database.close();
  assert.equal(verifySqliteBackupDatabase(ledgerFile).valid, true);
});
...
test('backup verification accepts a complete migration 064 ledger without migration 065', t => {
  ...
  database.exec(`
    ...
    DROP TABLE photo_evidence_captures;
    DROP TABLE photo_evidence_sets;
    DELETE FROM ledger_schema_migrations WHERE version IN ('065_governed_photo_evidence', '066_governed_client_feedback', '067_governed_energy_performance', '068_operational_safety_controls', '069_governed_framework_workspace', '070_managed_operator_accounts', '071_data_subject_request_governance', '072_operator_locale_preferences');
  `);
  database.close();
  assert.equal(verifySqliteBackupDatabase(ledgerFile).valid, true);
});
...
test('backup verification accepts a complete migration 065 ledger without migration 066', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-photo-evidence-compatibility-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Photo evidence compatibility fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec(`
    DROP TABLE energy_performance_records;
    DROP TABLE client_feedback;
    DELETE FROM ledger_schema_migrations WHERE version IN ('066_governed_client_feedback', '067_governed_energy_performance', '068_operational_safety_controls', '069_governed_framework_workspace', '070_managed_operator_accounts', '071_data_subject_request_governance', '072_operator_locale_preferences');
  `);
  database.close();
  assert.equal(verifySqliteBackupDatabase(ledgerFile).valid, true);
});

test('backup verification rejects a migration 066 ledger with missing client-feedback constraints', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-client-feedback-schema-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Client feedback schema fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec(`
    DROP INDEX idx_client_feedback_portal_survey;
    CREATE INDEX idx_client_feedback_portal_survey
      ON client_feedback(portal_access_id, survey_type);
  `);
  database.close();
  assert.throws(
    () => verifySqliteBackupDatabase(ledgerFile),
    /client-feedback constraints are incomplete: idx_client_feedback_portal_survey must be a partial unique index/i
  );
});

test('backup verification accepts a complete migration 066 ledger without migration 067', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-client-feedback-compatibility-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Client feedback compatibility fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec(`
    DROP TABLE energy_performance_records;
    DELETE FROM ledger_schema_migrations WHERE version IN ('067_governed_energy_performance', '068_operational_safety_controls', '069_governed_framework_workspace', '070_managed_operator_accounts', '071_data_subject_request_governance', '072_operator_locale_preferences');
  `);
  database.close();
  assert.equal(verifySqliteBackupDatabase(ledgerFile).valid, true);
});

test('backup verification rejects a migration 067 ledger with incomplete energy-performance constraints', t => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'contractor-ai-restore-energy-performance-schema-'));
  const ledgerFile = path.join(directory, 'ledger.sqlite');
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  createBackupLedger(ledgerFile, 'Energy-performance schema fixture');
  const database = new DatabaseSync(ledgerFile);
  database.exec(`
    DROP INDEX idx_energy_performance_current_scope;
    CREATE INDEX idx_energy_performance_current_scope
      ON energy_performance_records(job_id, phase, building_scope, object_reference);
  `);
  database.close();
  assert.throws(
    () => verifySqliteBackupDatabase(ledgerFile),
    /energy-performance constraints are incomplete: current scope record must be a partial unique index/i
  );
});
