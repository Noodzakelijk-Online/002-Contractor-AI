...
test('audit events append to one durable hash chain across a local restart', t => {
  ...
  try {
    ...
    assert.equal(restarted.migrationStatus().currentVersion, '022_controlled_document_revisions');
  } finally {
    restarted.close();
  }
});
...
test('migration 012 deterministically chains retained pre-chain audit history', t => {
  ...
  try {
    ...
    assert.equal(ledger.migrationStatus().currentVersion, '022_controlled_document_revisions');
    assert.deepEqual(
