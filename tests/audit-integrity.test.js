...
test('audit events append to one durable hash chain across a local restart', t => {
  ...
  try {
    ...
    assert.equal(restarted.migrationStatus().currentVersion, '015_invoice_issue_packages');
  } finally {
    restarted.close();
  }
});
...
test('migration 012 deterministically chains retained pre-chain audit history', t => {
  ...
  try {
    ...
    assert.equal(ledger.migrationStatus().currentVersion, '015_invoice_issue_packages');
    assert.deepEqual(
