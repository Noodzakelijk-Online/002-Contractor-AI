...
test('audit events append to one durable hash chain across a local restart', t => {
  ...
  try {
    ...
    assert.equal(restarted.migrationStatus().currentVersion, '052_governed_site_surveys');
  } finally {
    restarted.close();
  }
});
...
test('migration 012 deterministically chains retained pre-chain audit history', t => {
  ...
  try {
    ...
    assert.equal(ledger.migrationStatus().currentVersion, '052_governed_site_surveys');
    assert.deepEqual(
