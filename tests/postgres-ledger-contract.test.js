...
test('PostgreSQL data-subject requests preserve source-current approval and export parity', { skip: !connectionString }, () => {
  const ledger = new ContractorOperatingLedger({ databaseUrl: connectionString });
  try {
    const client = ledger.createClient({
      name: 'PostgreSQL privacy client',
      email: 'postgres-privacy@example.test',
      city: 'Utrecht'
    }, { actor: 'postgres_privacy_operator' });
    const request = ledger.createDataSubjectRequest({
      subjectType: 'client',
      subjectId: client.id,
      requestType: 'portability',
      channel: 'email',
      requesterReference: 'postgres-privacy-request-001'
    }, { actor: 'postgres_privacy_operator' });
    ledger.verifyDataSubjectRequestIdentity(request.id, {
      method: 'existing_contact',
      evidenceReference: 'Hosted retained client correspondence reference PRIV-PG-001'
    }, { actor: 'postgres_privacy_operator' });
    const assessed = ledger.assessDataSubjectRequest(request.id, {
      action: 'provide_portability',
      rationale: 'Prepare a structured owner-reviewed package for the verified requester.',
      legalBasisReference: 'GDPR Article 20 hosted review',
      retentionPolicyReference: 'Hosted retention policy 2026-01'
    }, { actor: 'postgres_privacy_owner' });
    ledger.resolveApproval(assessed.approval.id, {
      status: 'approved',
      resolvedBy: 'postgres_privacy_approver',
      reason: 'Hosted identity, scope, retained categories, and source-current inventory independently verified.'
    }, { actor: 'postgres_privacy_approver', enforceSeparation: true });

    const completed = ledger.getDataSubjectRequest(request.id);
    assert.equal(completed.status, 'completed');
    const exported = ledger.dataSubjectExportPayload(request.id);
    assert.equal(exported.payload.records.profile.email, client.email);
...
test('PostgreSQL adapter applies the ledger contract and durable scheduler migrations', { skip: !connectionString }, () => {
  ...
  try {
    ...
    assert.equal(migrations.currentVersion, '071_data_subject_request_governance');
