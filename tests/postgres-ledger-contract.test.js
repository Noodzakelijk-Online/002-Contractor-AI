...
test('PostgreSQL adapter applies the ledger contract and durable scheduler migrations', { skip: !connectionString }, () => {
  ...
  try {
    ...
    assert.equal(migrations.currentVersion, '051_governed_bid_decisions');
...
test('PostgreSQL startup lock serializes fresh concurrent replicas and releases after failure', { skip: !connectionString }, async () => {
  ...
  assert.deepEqual(versions, Array(4).fill('051_governed_bid_decisions'));
  ...
  try {
    ...
    assert.equal(Number(migrationCount.count), 51);
    ...
    `).rows[0];
    assert.equal(Number(opportunityTableCount.count), 6);
