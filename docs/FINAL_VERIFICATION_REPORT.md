...
| Node tests | Passed | Node 22.23.2 isolated suite: 533 tests, 497 passed, 36 PostgreSQL/environment skips, 0 failed, 86.1 s |
| Production build | Passed | `npm run build`: main application JS 531.51 kB and CSS 274.66 kB before gzip; the base locale remains 12.79 kB JS and the expanded 51.22 kB specialist operator catalog is loaded only when Dutch is active; client portal is 24.43 kB JS/10.19 kB CSS and job workspace controls are 233.70 kB JS |
| Bundle budget | Passed | `npm run verify:bundle`: largest JS 531,508 bytes, largest CSS 274,661 bytes, and 397,789 total gzip bytes across 39 assets; all budgets passed |
...
| Browser tests | Passed | All 90 Playwright Chromium workflows passed on current source in 23 bounded isolated batches in 496.8 s |
...
| In-app Browser QA | Passed against the current built runtime: persisted NL shell rendering, localized currency, compact navigation, no horizontal overflow, and a clean inspected error/warning log; ICP/service-area, bid/no-bid, WBS/quantity takeoff, labour-burden/overhead/unit-rate estimating, crew-capacity/two-week planning, Last Planner, 5S, LMRA, and 13-week cash-flow controls render in Dutch, including structured system warnings, while the complete portal, Performance Scorecard, Framework Register, site-survey, and practical-core persistence flows passed in Playwright |
...
- The operator shell, complete client portal, Performance Scorecard, Framework
  ...
  labour-burden/overhead/unit-rate estimating, crew-capacity/two-week planning,
  Last Planner, 5S, LMRA, and 13-week cash-flow controls now have
