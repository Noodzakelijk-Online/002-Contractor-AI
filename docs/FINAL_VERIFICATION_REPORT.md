...
| Node tests | Passed | Node 22.23.2 isolated suite: 533 tests, 497 passed, 36 PostgreSQL/environment skips, 0 failed, 95.8 s |
| Production build | Passed | `npm run build`: main application JS 531.95 kB and CSS 274.66 kB before gzip; the base locale remains 12.79 kB JS and the expanded 73.20 kB specialist operator catalog is loaded only when Dutch is active; client portal is 24.43 kB JS/10.19 kB CSS and job workspace controls are 235.25 kB JS |
| Bundle budget | Passed | `npm run verify:bundle`: largest JS 531,954 bytes, largest CSS 274,661 bytes, and 404,429 total gzip bytes across 39 assets; all budgets passed |
| Production-scale ledger | Passed | Deterministic 57,500-row production profile: seed 1,221.57 ms; startup 242.39 ms; reopen 4.44 ms; dashboard p95 475.92 ms; intake p95 55.59 ms; audit verification 308.86 ms; all correctness, resource, response-size, and latency thresholds passed |
| Browser tests | Passed | All 90 Playwright Chromium workflows passed on current source in 23 bounded isolated batches in 481.0 s |
...
| In-app Browser QA | Passed against the current built runtime: persisted NL shell rendering, localized currency, compact navigation, no horizontal overflow, and a clean inspected error/warning log; ICP/service-area, bid/no-bid, WBS/quantity takeoff, labour-burden/overhead/unit-rate estimating, commercial scope/allowances, project risk/premortem, fixed-price-versus-regie decisions, estimates and formal variations, crew-capacity/two-week planning, Last Planner, 5S, LMRA, and 13-week cash-flow controls render in Dutch, including structured system warnings, while the complete portal, Performance Scorecard, Framework Register, site-survey, and practical-core persistence flows passed in Playwright |
...
- The operator shell, complete client portal, Performance Scorecard, Framework
  ...
  quote acceptance, formal variations, crew-capacity/two-week planning, Last
  Planner, 5S, LMRA, and 13-week cash-flow controls now have
