...
| Node tests | Passed | Node 22.23.2 isolated suite: 533 tests, 497 passed, 36 PostgreSQL/environment skips, 0 failed, 613.8 s |
| Production build | Passed | `npm run build`: main application JS 530.84 kB and CSS 274.66 kB before gzip; the base locale remains 12.79 kB JS and the 16.14 kB specialist operator catalog is a lazy shared chunk; client portal is 24.43 kB JS/10.19 kB CSS and job workspace controls are 232.90 kB JS |
| Bundle budget | Passed | `npm run verify:bundle`: largest JS 530,839 bytes, largest CSS 274,661 bytes, and 384,272 total gzip bytes across 39 assets; all budgets passed |
...
| Browser tests | Passed | All 90 Playwright Chromium workflows passed on current source in 23 bounded isolated batches in 713.1 s |
...
| In-app Browser QA | Passed against the current built runtime: persisted NL shell rendering, localized currency, compact navigation, no horizontal overflow, and a clean inspected error/warning log; the ICP/service-area and bid/no-bid controls render in Dutch and survive reload, while the complete portal, Performance Scorecard, Framework Register, site-survey, and practical-core persistence flows passed in Playwright |
...
- The operator shell, complete client portal, Performance Scorecard, Framework
  Register, ICP/service-area, bid/no-bid, and site-survey controls now have
...
representative dashboard at 424.63 ms p95 and a canonical intake at 145.57 ms p95
