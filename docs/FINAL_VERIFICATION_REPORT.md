...
| Node tests | Passed | Node 22.23.2 isolated suite: 533 tests, 497 passed, 36 PostgreSQL/environment skips, 0 failed, 102.6 s |
| Production build | Passed | `npm run build`: main application JS 532.70 kB and CSS 274.66 kB before gzip; the base locale remains 12.79 kB JS and the expanded 92.76 kB specialist operator catalog is loaded only when Dutch is active; client portal is 24.43 kB JS/10.19 kB CSS and job workspace controls are 235.82 kB JS |
| Bundle budget | Passed | `npm run verify:bundle`: largest JS 532,699 bytes, largest CSS 274,661 bytes, and 410,160 total gzip bytes across 39 assets; all budgets passed |
...
| Browser tests | Passed | All 90 Playwright Chromium workflows passed on current source in 23 bounded isolated batches in 499.8 s |
...
- The operator shell, complete client portal, Performance Scorecard, Framework
  ...
  quantity/acknowledgement/conversion, Last Planner, 5S, LMRA, and 13-week cash-flow controls now have
