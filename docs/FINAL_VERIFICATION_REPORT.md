...
| Release contract | Passed | `npm run verify:release`: 61 canonical paths, 12 retired paths, 16 hosted keys, 319 canonical source files; generated release/runtime/artifact directories excluded |
...
| Node tests | Passed | `npm test`: 527 tests, 491 passed, 36 PostgreSQL/environment skips, 0 failed, 300.5 s |
...
| Production-scale ledger | Passed | Deterministic 63,500-row production profile: seed 1,526.69 ms; cold start 760.83 ms; reopen 5.41 ms; dashboard p95 544.67 ms; intake p95 51.51 ms; audit verification 251.96 ms; all correctness, resource, response-size, and latency thresholds passed |
| Browser tests | Passed | All 89 Playwright Chromium workflows passed on current source in 23 bounded isolated batches in 583.0 s |
...
| Local runtime | Passed | The retained standalone remains ready at `127.0.0.1:4175`; the current final build was also served from an isolated local production runtime for in-app Browser QA, and both public readiness probes returned 200 |
...
| Audit actor integrity | Passed for authenticated role principals, scoped client portal identity, and local `local:owner`; all 164 explicit mutation actor sites consume the trusted principal directly, the release contract rejects body-derived actors, and caller labels are excluded from audit history plus retained and cloned input paths |
...
representative dashboard at 400.98 ms p95 and a canonical intake at 56.72 ms p95
