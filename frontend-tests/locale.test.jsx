...
describe('locale contract', () => {
  ...
  it('translates specialist operator controls and interpolates runtime values', () => {
    ...
    expect(operatorText('nl-NL', 'Equipment handoff')).toBe('Materieeloverdracht')
    expect(operatorText('nl-NL', '{name} returned as {condition} and moved to quarantine review.', { name: 'Laser 4', condition: 'beschadigd' }))
      .toBe('Laser 4 is ingeleverd als beschadigd en verplaatst naar quarantainebeoordeling.')
    expect(operatorText('nl-NL', 'Approval cannot complete while {count} operational reservations remain.', { count: 2 }))
      .toBe('Goedkeuring kan niet worden voltooid zolang 2 operationele reserveringen actief zijn.')
    expect(operatorText('nl-NL', 'reinspection required')).toBe('herinspectie vereist')
    expect(operatorText('nl-NL', 'Business identity')).toBe('Bedrijfsidentiteit')
    expect(operatorText('nl-NL', '{section} details saved to the operating ledger.', { section: 'Identiteit' }))
      .toBe('Gegevens voor Identiteit zijn opgeslagen in het bedrijfsregister.')
    expect(operatorText('nl-NL', 'Team access')).toBe('Teamtoegang')
    expect(operatorText('nl-NL', 'Access key for {name}', { name: 'Operator Noord' })).toBe('Toegangssleutel voor Operator Noord')
    expect(operatorText('nl-NL', 'Suspend autonomous drafting')).toBe('Autonoom opstellen opschorten')
    expect(operatorText('nl-NL', 'Archive {count} record(s)', { count: 4 })).toBe('4 record(s) archiveren')
