...
import {
  Activity, Archive, ArrowUpRight, BadgeCheck, Ban, Bell, BriefcaseBusiness, Building2,
  CalendarDays, Check, ChevronRight, ClipboardCheck, ClipboardList, CloudOff, FileDown, FileUp,
  Copy, Eye, EyeOff, FolderArchive, Gauge, HardHat, LayoutDashboard, Link2, LoaderCircle, LockKeyhole, LogOut, MapPin, MessageSquareText,
  Menu, PackageCheck, Pencil, Plus, ReceiptEuro, RefreshCw, Search, ShieldCheck, TriangleAlert, Users,
  Wrench, X
} from 'lucide-react'
import {
  createFieldEvidenceDraftId, enqueueFieldEvidenceDraft, enqueueFieldOperationDraft, fieldOutboxAvailable,
  fieldOutboxOperatorScope, fieldOutboxSnapshot, flushFieldOutbox, shouldQueueFieldMutation
} from './field-outbox'
import './App.css'
...
function formatDate(value) {
  ...
  return Number.isNaN(parsed.getTime()) ? String(value) : new Intl.DateTimeFormat('nl-NL', { day: 'numeric', month: 'short' }).format(parsed)
}

function formatDateTime(value) {
  if (!value) return 'Not retained'
  const parsed = new Date(value)
  return Number.isNaN(parsed.getTime()) ? String(value) : new Intl.DateTimeFormat('nl-NL', {
    day: 'numeric',
    month: 'short',
    year: 'numeric',
    hour: '2-digit',
...
function emptyEquipmentDraft(tool = null) {
  ...
  return {
    ...
    status: EQUIPMENT_EDITABLE_STATUSES.has(retainedStatus)
      ? retainedStatus
      : retainedStatus === 'retired' ? 'available' : 'in_use',
...
function Empty({ title, detail }) {
  return <div className="empty"><BadgeCheck size={22} /><strong>{title}</strong><span>{detail}</span></div>
}

function ApprovalQueueItem({ item, submitting, onReview }) {
  const decision = item.decision || {}
  return <article className="approval-item">
    <div className="approval-icon"><ShieldCheck size={19} /></div>
    <div className="approval-copy">
      <div className="approval-title-row"><h3>{item.title || item.summary || formatStatus(item.targetType)}</h3><span className={`tag tag-${decision.riskLevel === 'high' ? 'amber' : 'green'}`}>{decision.riskLevel || 'review'}</span></div>
      <p>{decision.primaryEffect || item.summary || item.reason || 'Review this retained ledger decision.'}</p>
      <small>{item.jobTitle || item.jobId || 'Portfolio decision'} / {formatDate(item.createdAt)}</small>
    </div>
    <div className="approval-actions"><button className="secondary-button" disabled={submitting} onClick={() => onReview(item, 'rejected')}>Reject</button><button className="primary-button" disabled={submitting} onClick={() => onReview(item, 'approved')}>Review and approve</button></div>
  </article>
}

function DispatchWorkspace({ dispatch, jobs, canCoordinate, canApprove, submitting, onPrepare, onControl, onReviewWorkforce, onReviewEquipment, onOpenApprovals, onOpen }) {
  const rows = dispatch?.jobs || dispatch?.rows || dispatch?.dispatch || EMPTY_LIST
  const summary = dispatch?.summary || {}
  return <section className="panel page-panel dispatch-workspace" data-testid="dispatch-workspace">
    <div className="panel-heading"><div><h2>Dispatch readiness</h2><p>Mobilization stays blocked until assignments, access, safety, equipment, and approval controls are ready.</p></div><span className="count-badge">{rows.length}</span></div>
    <div className="dispatch-summary" aria-label="Dispatch summary">
      <div><span>Ready</span><strong>{summary.ready || 0}</strong></div>
      <div><span>Needs plan</span><strong>{summary.needsPlan || 0}</strong></div>
      <div><span>Blocked</span><strong>{summary.blocked || 0}</strong></div>
      <div><span>Approvals</span><strong>{summary.pendingApprovals || 0}</strong></div>
    </div>
    <div className="dispatch-list">{rows.map(item => {
      const job = jobs.find(candidate => candidate.id === item.jobId) || { id: item.jobId, title: item.jobTitle || item.title || 'Ledger job' }
      const controlActions = (item.nextActions || []).filter(action => action.recordType && action.recordId).slice(0, 4)
      const workforceBlocker = (item.blockers || []).find(blocker => ['worker_record_missing', 'worker_retirement_pending', 'worker_unavailable', 'worker_conflict'].includes(blocker.type))
      const equipmentBlocker = (item.blockers || []).find(blocker => ['tool_record_missing', 'tool_retirement_pending', 'tool_inspection_readiness', 'tool_unavailable'].includes(blocker.type))
      return <article className="dispatch-item" key={item.jobId || item.id}>
        <div className="dispatch-copy"><div className="dispatch-title"><h3>{item.jobTitle || item.title || 'Ledger job'}</h3><span className={`status status-${item.readinessStatus || item.status}`}>{formatStatus(item.readinessStatus || item.status)}</span></div><p>{item.nextAction || item.summary || item.message || 'Review readiness controls before dispatch.'}</p><div className="dispatch-flags">{item.blockers?.length ? <span className="tag tag-amber">{item.blockers.length} blocker{item.blockers.length === 1 ? '' : 's'}</span> : null}{item.missing?.length ? <span className="tag">{item.missing.length} plan gap{item.missing.length === 1 ? '' : 's'}</span> : null}{item.counts?.activeAssignments ? <span className="tag tag-green">{item.counts.activeAssignments} assigned</span> : null}</div></div>
        <div className="dispatch-actions">
          {item.counts?.pendingApprovals && canApprove ? <button className="secondary-button" aria-label={`Review approvals for ${job.title}`} disabled={submitting} onClick={() => onOpenApprovals({ jobId: item.jobId, jobTitle: job.title, approvalId: item.nextActions?.find(action => action.approvalId)?.approvalId || null })}><ShieldCheck size={16} />Review approval</button> : null}
          {canCoordinate ? controlActions.map(action => {
            const actionLabel = action.actionLabel || 'Resolve dispatch control'
            return <button key={`${action.type}-${action.recordType}-${action.recordId}`} className="secondary-button" aria-label={`${actionLabel} for ${job.title}`} disabled={submitting} onClick={() => onControl(item, action)}><ClipboardCheck size={16} />{actionLabel}</button>
          }) : null}
          {workforceBlocker ? <button className="secondary-button" aria-label={`Review crew for ${job.title}`} disabled={submitting} onClick={() => onReviewWorkforce(workforceBlocker)}><Users size={16} />Review crew</button> : null}
          {equipmentBlocker ? <button className="secondary-button" aria-label={`Review equipment for ${job.title}`} disabled={submitting} onClick={() => onReviewEquipment(equipmentBlocker)}><Wrench size={16} />Review equipment</button> : null}
          {canCoordinate ? <button className="secondary-button" aria-label={`Prepare internal pack for ${job.title}`} disabled={submitting} onClick={() => onPrepare(item)}><PackageCheck size={16} />Prepare internal pack</button> : null}
          <button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => onOpen(job)}><ArrowUpRight size={16} /></button>
        </div>
      </article>
    })}{!rows.length ? <Empty title="No active dispatch work" detail="Active ledger jobs will appear here with their retained readiness controls." /> : null}</div>
  </section>
}
...
function WorkerDirectory({ workers, summary, canCoordinate, canApprove, submitting, onCreate, onEdit, onRetire, onOpenApprovals }) {
  ...
  return <div className="trade-partner-directory worker-directory" data-testid="worker-directory" role="tabpanel">
    <div className="resource-summary worker-summary" aria-label="Crew summary">
      <div><span>Active crew</span><strong>{summary?.active || 0}</strong></div>
      <div><span>Available</span><strong>{summary?.available || 0}</strong></div>
      <div><span>Unavailable</span><strong>{summary?.unavailable || 0}</strong></div>
      <div><span>Retirement review</span><strong>{summary?.pendingRetirement || 0}</strong></div>
    </div>
    <div className="trade-partner-toolbar worker-toolbar">
      <label className="search-control"><Search size={16} /><span className="visually-hidden">Search crew directory</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search crew" /></label>
      <div className="resource-tabs" role="tablist" aria-label="Crew status">
        {[['active', 'Active'], ['unavailable', 'Unavailable'], ['retired', 'Retired']].map(([key, label]) => <button key={key} role="tab" aria-selected={filter === key} className={filter === key ? 'resource-tab-active' : ''} onClick={() => setFilter(key)}>{label}</button>)}
      </div>
      {canCoordinate ? <button className="primary-button" disabled={submitting} onClick={onCreate}><Plus size={16} />Add crew member</button> : null}
    </div>
    <div className="trade-partner-list worker-list">
      {visibleWorkers.map(worker => <article className="trade-partner-row worker-row" key={worker.id}>
        <div className="trade-partner-identity worker-identity"><span className="partner-icon"><HardHat size={18} /></span><div><div className="trade-partner-title worker-title"><h3>{worker.name}</h3><span className={`status status-${worker.status}`}>{formatStatus(worker.status)}</span>{worker.retirementApprovalId ? <span className="tag tag-amber">Retirement pending</span> : null}</div><p>{worker.role || 'Role not retained'}{worker.skills?.length ? ` / ${worker.skills.slice(0, 3).join(', ')}` : ''}</p><small>{worker.email || worker.phone || 'Contact not retained'}</small></div></div>
        <div className="trade-partner-evidence worker-details"><span>Home region <strong>{worker.homeRegion || 'Not retained'}</strong></span><span>Hourly cost <strong>{currency.format(worker.hourlyRate || 0)}</strong></span><span>Active assignments <strong>{worker.activeAssignmentCount || 0}</strong></span><span>Last updated <strong>{formatDate(worker.updatedAt)}</strong></span>{worker.dormantAssignmentCount ? <span>Dormant assignments <strong>{worker.dormantAssignmentCount}</strong></span> : null}</div>
        <div className="trade-partner-actions worker-actions">
          {worker.retirementApprovalId && canApprove ? <button className="secondary-button" disabled={submitting} onClick={() => onOpenApprovals({ approvalId: worker.retirementApprovalId })}><ShieldCheck size={15} />Review retirement</button> : null}
          {canCoordinate && worker.status !== 'retired' ? <button className="icon-button" aria-label={`Edit ${worker.name}`} disabled={submitting} onClick={() => onEdit(worker)}><Pencil size={16} /></button> : null}
          {canCoordinate && worker.status !== 'retired' ? <button className="icon-button danger-icon" aria-label={`Request retirement for ${worker.name}`} disabled={submitting || Boolean(worker.retirementApprovalId)} onClick={() => onRetire(worker)}><Archive size={16} /></button> : null}
        </div>
      </article>)}
      {!visibleWorkers.length ? <Empty title="No matching crew members" detail="Retained people records matching this view will appear here." /> : null}
    </div>
  </div>
}

function EquipmentDirectory({ tools, summary, canCoordinate, canApprove, submitting, onCreate, onEdit, onInspect, onMaintain, onRetire, onOpenApprovals }) {
  const [query, setQuery] = useState('')
  const [filter, setFilter] = useState('active')
  const visibleTools = useMemo(() => {
    ...
    return (tools || []).filter(tool => {
      ...
      if (filter === 'attention' && ['available', 'retired'].includes(tool.status) && !tool.retirementApprovalId && !tool.inspection?.requiresAttention && !tool.maintenance?.requiresAttention) return false
  ...
  return <div className="trade-partner-directory equipment-directory" data-testid="equipment-directory" role="tabpanel">
    <div className="resource-summary equipment-summary" aria-label="Equipment summary">
      <div><span>Active equipment</span><strong>{summary?.active || 0}</strong></div>
      <div><span>Available</span><strong>{summary?.available || 0}</strong></div>
      <div><span>Attention</span><strong>{summary?.attention || 0}</strong></div>
      <div><span>Retirement review</span><strong>{summary?.pendingRetirement || 0}</strong></div>
    </div>
    <div className="trade-partner-toolbar equipment-toolbar">
      <label className="search-control"><Search size={16} /><span className="visually-hidden">Search equipment directory</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search equipment" /></label>
      <div className="resource-tabs" role="tablist" aria-label="Equipment status">
        {[['active', 'Active'], ['attention', 'Attention'], ['retired', 'Retired']].map(([key, label]) => <button key={key} role="tab" aria-selected={filter === key} className={filter === key ? 'resource-tab-active' : ''} onClick={() => setFilter(key)}>{label}</button>)}
      </div>
      {canCoordinate ? <button className="primary-button" disabled={submitting} onClick={onCreate}><Plus size={16} />Add equipment</button> : null}
    </div>
    <div className="trade-partner-list equipment-list">
      {visibleTools.map(tool => <article className="trade-partner-row equipment-row" key={tool.id}>
        <div className="trade-partner-identity equipment-identity"><span className="partner-icon"><Wrench size={18} /></span><div><div className="trade-partner-title equipment-title"><h3>{tool.name}</h3><span className={`status status-${tool.status}`}>{formatStatus(tool.status)}</span>{tool.inspection?.required ? <span className={`tag ${tool.inspection.requiresAttention ? 'tag-amber' : 'tag-green'}`}>Inspection {formatStatus(tool.inspection.status)}</span> : null}{tool.retirementApprovalId ? <span className="tag tag-amber">Retirement pending</span> : null}</div><p>{formatStatus(tool.category || 'general equipment')}</p><small>{tool.data?.serialNumber || tool.currentLocation || tool.homeLocation || 'Reference not retained'}</small></div></div>
        <div className="trade-partner-evidence equipment-details"><span>Current location <strong>{tool.currentLocation || 'Not retained'}</strong></span><span>Inspection due <strong>{tool.inspection?.required ? formatDate(tool.inspection.dueAt) : 'Not required'}</strong></span><span>Last inspection <strong>{tool.inspection?.lastInspectedAt ? `${formatDate(tool.inspection.lastInspectedAt)} / ${formatStatus(tool.inspection.lastResult)}` : 'Not retained'}</strong></span><span>Last maintenance <strong>{tool.maintenance?.lastMaintainedAt ? `${formatDate(tool.maintenance.lastMaintainedAt)} / ${formatStatus(tool.maintenance.latestMaintenance?.outcome)}` : 'Not retained'}</strong></span><span>Active reservations <strong>{tool.activeReservationCount || 0}</strong></span><span>Last updated <strong>{formatDate(tool.updatedAt)}</strong></span>{tool.dormantReservationCount ? <span>Dormant reservations <strong>{tool.dormantReservationCount}</strong></span> : null}</div>
        <div className="trade-partner-actions equipment-actions">
          {tool.retirementApprovalId && canApprove ? <button className="secondary-button" disabled={submitting} onClick={() => onOpenApprovals({ approvalId: tool.retirementApprovalId })}><ShieldCheck size={15} />Review retirement</button> : null}
          {canCoordinate && tool.status !== 'retired' && !tool.retirementApprovalId ? <button className="secondary-button" aria-label={`Record maintenance for ${tool.name}`} disabled={submitting} onClick={() => onMaintain(tool)}><Wrench size={15} />Maintain</button> : null}
          {canCoordinate && tool.status !== 'retired' && !tool.retirementApprovalId ? <button className="secondary-button" aria-label={`Record inspection for ${tool.name}`} disabled={submitting} onClick={() => onInspect(tool)}><ClipboardCheck size={15} />Inspect</button> : null}
          {canCoordinate && tool.status !== 'retired' ? <button className="icon-button" aria-label={`Edit ${tool.name}`} disabled={submitting} onClick={() => onEdit(tool)}><Pencil size={16} /></button> : null}
          {canCoordinate && tool.status !== 'retired' ? <button className="icon-button danger-icon" aria-label={`Request retirement for ${tool.name}`} disabled={submitting || Boolean(tool.retirementApprovalId)} onClick={() => onRetire(tool)}><Archive size={16} /></button> : null}
        </div>
      </article>)}
      {!visibleTools.length ? <Empty title="No matching equipment" detail="Retained equipment records matching this view will appear here." /> : null}
    </div>
  </div>
}
...
function TradePartnerDirectory({ partners, summary, canCoordinate, canApprove, submitting, onCreate, onEdit, onRetire, onOpenApprovals }) {
  ...
  return <div className="trade-partner-directory" data-testid="trade-partner-directory">
    <div className="resource-summary trade-partner-summary" aria-label="Trade partner summary">
      ...
      <div><span>Verified</span><strong>{summary?.verified || 0}</strong></div>
      <div><span>Expiring</span><strong>{summary?.expiring || 0}</strong></div>
      <div><span>Action required</span><strong>{summary?.actionRequired || 0}</strong></div>
    </div>
    <div className="trade-partner-toolbar">
      <label className="search-control"><Search size={16} /><span className="visually-hidden">Search trade partners</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search partners" /></label>
      <div className="resource-tabs" role="tablist" aria-label="Trade partner status">
        {[['active', 'Active'], ['attention', 'Attention'], ['retired', 'Retired']].map(([key, label]) => <button key={key} role="tab" aria-selected={filter === key} className={filter === key ? 'resource-tab-active' : ''} onClick={() => setFilter(key)}>{label}</button>)}
      </div>
      {canCoordinate ? <button className="primary-button" disabled={submitting} onClick={onCreate}><Plus size={16} />Add trade partner</button> : null}
    </div>
    <div className="trade-partner-list">
      {visiblePartners.map(partner => {
        const compliance = partner.compliance || { status: 'needs_review', blockers: [], warnings: [] }
        const evidenceMessage = compliance.blockers?.[0]?.message || compliance.warnings?.[0]?.message || 'Required partner evidence is current.'
        return <article className="trade-partner-row" key={partner.id}>
          <div className="trade-partner-identity"><span className="partner-icon"><Building2 size={18} /></span><div><div className="trade-partner-title"><h3>{partner.name}</h3><span className={`status status-${compliance.status}`}>{formatStatus(compliance.status)}</span></div><p>{formatStatus(partner.partnerType)}{partner.specialties?.length ? ` · ${partner.specialties.slice(0, 3).join(', ')}` : ''}</p><small>{partner.contactName || partner.email || partner.city || 'Contact not retained'}</small></div></div>
          <div className="trade-partner-evidence"><span>Registration <strong>{partner.registrationNumber || 'Missing'}</strong></span><span>VAT <strong>{partner.data?.vatExempt ? 'Exempt' : partner.vatNumber || 'Missing'}</strong></span><p>{evidenceMessage}</p></div>
          <div className="trade-partner-actions">
            {partner.retirementApprovalId && canApprove ? <button className="secondary-button" disabled={submitting} onClick={() => onOpenApprovals({ approvalId: partner.retirementApprovalId })}><ShieldCheck size={15} />Review retirement</button> : null}
            {canCoordinate && partner.status !== 'retired' ? <button className="icon-button" aria-label={`Edit ${partner.name}`} disabled={submitting} onClick={() => onEdit(partner)}><Pencil size={16} /></button> : null}
            {canCoordinate && partner.status !== 'retired' ? <button className="icon-button danger-icon" aria-label={`Request retirement for ${partner.name}`} disabled={submitting || Boolean(partner.retirementApprovalId)} onClick={() => onRetire(partner)}><Archive size={16} /></button> : null}
          </div>
        </article>
      })}
      {!visiblePartners.length ? <Empty title="No matching trade partners" detail="Supplier and subcontractor records matching this view will appear here." /> : null}
    </div>
  </div>
}

function ResourcesWorkspace({ workforce, inventory, workers, workerSummary, tools, toolSummary, tradePartners, tradePartnerSummary, jobs, view, onViewChange, canCoordinate, canApprove, submitting, onPlan, onDraftInstruction, onReviewCrewEvidence, onPrepareLoading, onDraftProcurement, onAction, onCreateWorker, onEditWorker, onRetireWorker, onCreateEquipment, onEditEquipment, onInspectEquipment, onMaintainEquipment, onRetireEquipment, onCreatePartner, onEditPartner, onRetirePartner, onOpenApprovals, onOpen }) {
  ...
  return <section className="panel page-panel resources-workspace" data-testid="resources-workspace">
    <div className="panel-heading resources-heading">
      <div><h2>Resource readiness</h2><p>{isPartners ? 'Retain supplier and subcontractor identity, compliance, and expiry evidence before purchasing approval.' : isEquipment ? 'Maintain retained equipment identity, condition, location, reservation, and retirement safeguards.' : isCrewDirectory ? 'Maintain retained crew identity, availability, skills, cost, and assignment safeguards.' : 'Coordinate retained crew, equipment, material, procurement, and loading records before work is committed.'}</p></div>
      <div className="resource-tabs" role="tablist" aria-label="Resource readiness view">
        <button role="tab" aria-selected={isWorkforce} className={isWorkforce ? 'resource-tab-active' : ''} onClick={() => onViewChange('workforce')}><Users size={15} />Workforce</button>
        <button role="tab" aria-selected={isInventory} className={isInventory ? 'resource-tab-active' : ''} onClick={() => onViewChange('inventory')}><PackageCheck size={15} />Inventory</button>
        <button role="tab" aria-selected={isEquipment} className={isEquipment ? 'resource-tab-active' : ''} onClick={() => onViewChange('equipment')}><Wrench size={15} />Equipment</button>
        <button role="tab" aria-selected={isPartners} className={isPartners ? 'resource-tab-active' : ''} onClick={() => onViewChange('partners')}><Building2 size={15} />Trade partners</button>
      </div>
    </div>
    {isWorkforce ? <div className="workforce-mode-toolbar"><div className="resource-tabs" role="tablist" aria-label="Workforce workspace mode"><button role="tab" aria-selected={workforceMode === 'readiness'} className={workforceMode === 'readiness' ? 'resource-tab-active' : ''} onClick={() => setWorkforceMode('readiness')}><ClipboardCheck size={15} />Readiness</button><button role="tab" aria-selected={workforceMode === 'crew'} className={workforceMode === 'crew' ? 'resource-tab-active' : ''} onClick={() => setWorkforceMode('crew')}><Users size={15} />Crew directory</button></div></div> : null}
    {isPartners ? <TradePartnerDirectory partners={tradePartners} summary={tradePartnerSummary} canCoordinate={canCoordinate} canApprove={canApprove} submitting={submitting} onCreate={onCreatePartner} onEdit={onEditPartner} onRetire={onRetirePartner} onOpenApprovals={onOpenApprovals} /> : isEquipment ? <EquipmentDirectory tools={tools} summary={toolSummary} canCoordinate={canCoordinate} canApprove={canApprove} submitting={submitting} onCreate={onCreateEquipment} onEdit={onEditEquipment} onInspect={onInspectEquipment} onMaintain={onMaintainEquipment} onRetire={onRetireEquipment} onOpenApprovals={onOpenApprovals} /> : isCrewDirectory ? <WorkerDirectory workers={workers} summary={workerSummary} canCoordinate={canCoordinate} canApprove={canApprove} submitting={submitting} onCreate={onCreateWorker} onEdit={onEditWorker} onRetire={onRetireWorker} onOpenApprovals={onOpenApprovals} /> : <><div className="resource-summary" aria-label={`${isWorkforce ? 'Workforce' : 'Inventory'} summary`}>
      {isWorkforce ? <>
        <div><span>Needs crew</span><strong>{summary.needsAssignment || 0}</strong></div>
        <div><span>Conflicts</span><strong>{summary.workerConflicts || 0}</strong></div>
        <div><span>Instructions</span><strong>{summary.needsInstruction || 0}</strong></div>
        <div><span>Site access</span><strong>{summary.siteAccess || 0}</strong></div>
      </> : <>
        <div><span>Procurement</span><strong>{summary.procurementNeeded || 0}</strong></div>
        <div><span>Partner holds</span><strong>{summary.partnerComplianceBlocks || 0}</strong></div>
        <div><span>Loading gaps</span><strong>{summary.loadingMissing || 0}</strong></div>
        <div><span>Approvals</span><strong>{summary.pendingApprovals || 0}</strong></div>
      </>}
    </div>
    <div className="resource-readiness-list" role="tabpanel">
      {rows.map(item => {
        ...
        const canPlan = canAct && (isWorkforce
          ? item.flags?.needsAssignment || item.flags?.workerConflict || item.flags?.offlineAssigned
          : item.flags?.toolConflict)
        const canDraftInstruction = canAct && isWorkforce && item.nextActions?.some(action => action.type === 'draft_worker_instruction')
        ...
        const crewReviewActions = canAct && isWorkforce ? item.nextActions?.filter(action => ['publish_worker_instruction', 'clear_site_access'].includes(action.type)) || [] : []
        return <article className="resource-readiness-item" key={`${view}-${item.jobId}`}>
          <div className="resource-readiness-copy">
            <div className="resource-readiness-title"><h3>{item.jobTitle || 'Ledger job'}</h3><span className={`status status-${status}`}>{formatStatus(status)}</span></div>
            <p>{item.nextAction || 'Resource records are stable.'}</p>
            {isWorkforce ? <div className="resource-readiness-values">
              <span>Assigned <strong>{item.counts?.activeAssignments || 0}</strong></span>
              <span>Conflicts <strong>{item.counts?.workerConflicts || 0}</strong></span>
              <span>Unavailable <strong>{item.counts?.offlineAssignments || 0}</strong></span>
              <span>Instructions <strong>{item.counts?.workerInstructions || 0}</strong></span>
              <span>Hours logged <strong>{item.counts?.billableHours || 0}</strong></span>
            </div> : <div className="resource-readiness-values">
              <span>Open materials <strong>{item.counts?.openMaterials || 0}</strong></span>
              <span>Reserved tools <strong>{item.counts?.toolReservations || 0}</strong></span>
              <span>Load items <strong>{item.counts?.loadingItems || 0}</strong></span>
              <span>Material cost <strong>{currency.format(item.money?.materialCost || 0)}</strong></span>
            </div>}
            <div className="resource-readiness-flags">
              {item.counts?.pendingApprovals ? <span className="tag tag-amber">{item.counts.pendingApprovals} approval{item.counts.pendingApprovals === 1 ? '' : 's'}</span> : null}
              {!isWorkforce && item.counts?.partnerComplianceBlocks ? <span className="tag tag-amber">{item.counts.partnerComplianceBlocks} partner hold{item.counts.partnerComplianceBlocks === 1 ? '' : 's'}</span> : null}
              {isWorkforce && item.counts?.dueOrientations ? <span className="tag tag-amber">{item.counts.dueOrientations} orientation due</span> : null}
              {isWorkforce && item.counts?.staleCrewEvidence ? <span className="tag">{item.counts.staleCrewEvidence} historical crew record{item.counts.staleCrewEvidence === 1 ? '' : 's'}</span> : null}
              {!isWorkforce && item.counts?.dueMaterials ? <span className="tag tag-amber">{item.counts.dueMaterials} material item{item.counts.dueMaterials === 1 ? '' : 's'} due</span> : null}
          ...
          <div className="resource-readiness-actions">
            ...
            {!isWorkforce && item.flags?.supplierComplianceBlocked && canCoordinate ? <button className="secondary-button" disabled={submitting} onClick={() => onViewChange('partners')}><Building2 size={16} />Trade partners</button> : null}
            {canPlan ? <button className="secondary-button" disabled={submitting} onClick={() => onPlan(item)}><Users size={16} />Plan resources</button> : null}
            {canDraftInstruction ? <button className="secondary-button" aria-label={`Draft crew instructions for ${job.title}`} disabled={submitting} onClick={() => onDraftInstruction(item)}><ClipboardCheck size={16} />Draft instructions</button> : null}
            {crewReviewActions.map(action => <button className="secondary-button" key={`${action.type}-${action.recordId || action.assignmentId || action.workerId}`} aria-label={`${action.label} for ${job.title}`} disabled={submitting} onClick={() => onReviewCrewEvidence(item, action)}><ShieldCheck size={16} />{action.type === 'publish_worker_instruction' ? 'Review instructions' : 'Clear site access'}</button>)}
            {canDraftProcurement ? <button className="secondary-button" aria-label={`Draft procurement for ${job.title}`} disabled={submitting} onClick={() => onDraftProcurement(item)}><ReceiptEuro size={16} />Draft procurement</button> : null}
            {canPrepareLoading ? <button className="secondary-button" aria-label={`Prepare loading checklist for ${job.title}`} disabled={submitting} onClick={() => onPrepareLoading(item)}><PackageCheck size={16} />Loading checklist</button> : null}
            {resourceAction && resourceActionLabel ? <button className="secondary-button" aria-label={`${resourceActionLabel} for ${job.title}`} disabled={submitting} onClick={() => onAction(item, resourceAction)}><ClipboardCheck size={16} />{resourceActionLabel}</button> : null}
            <button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => onOpen(job)}><ArrowUpRight size={16} /></button>
          </div>
        </article>
      })}
      {!rows.length ? <Empty title={isWorkforce ? 'No workforce work' : 'No inventory work'} detail={isWorkforce ? 'Scheduled ledger jobs will appear here with crew readiness and time controls.' : 'Material and equipment requirements will appear here with procurement and loading controls.'} /> : null}
    </div></>}
  </section>
}
...
function FinanceWorkspace({ finance, jobs, canCoordinate, canApprove, submitting, onDraftInvoice, onPrepareInvoice, onAction, onOpenApprovals, onOpen }) {
  ...
  return <section className="panel page-panel finance-workspace" data-testid="finance-workspace">
    ...
    <div className="finance-summary" aria-label="Finance summary">
      ...
      <div><span>Planned billing</span><strong>{currency.format(summary.plannedBillingValue || 0)}</strong></div>
    ...
    <div className="finance-list">{rows.map(item => {
      ...
      const prepareAction = canAct ? item.nextActions?.find(action => ['prepare_invoice_package', 'prepare_credit_note_package'].includes(action.type)) : null
    ...
    })}{!rows.length ? <Empty title="No finance work" detail="Active and completed ledger jobs will appear here with their finance readiness state." /> : null}</div>
  </section>
}
...
function ClientSuccessWorkspace({ clients, jobs, canCoordinate, canApprove, submitting, onPrepareCloseout, onPrepareHandover, onDraftFollowup, onDraftRecurring, onLifecycle, onOpenApprovals, onOpen }) {
  ...
  return <section className="panel page-panel client-workspace" data-testid="client-workspace">
    ...
    <div className="client-list">{rows.map(item => {
      const job = jobs.find(candidate => candidate.id === item.jobId) || { id: item.jobId, title: item.jobTitle || 'Ledger job' }
      ...
      return <article className="client-item" key={item.jobId}>
        <div className="client-copy">
          <div className="client-title"><h3>{item.jobTitle || 'Ledger job'}</h3><span className={`status status-${item.clientStatus}`}>{formatStatus(item.clientStatus)}</span></div>
          <p>{item.nextAction || 'Client records are stable.'}</p>
          <div className="client-values">
            <span>Client value <strong>{currency.format(item.money?.clientValue || 0)}</strong></span>
            <span>Selections <strong>{item.counts?.pendingSelections || 0}</strong></span>
            <span>Waiting replies <strong>{item.counts?.waitingReplies || 0}</strong></span>
            <span>Open service <strong>{(item.counts?.openPunchItems || 0) + (item.counts?.openWarrantyClaims || 0) + (item.counts?.openAftercare || 0)}</strong></span>
          </div>
        ...
        <div className="client-actions">
          ...
          {warrantyAction ? <button className="secondary-button" aria-label={`Request warranty resolution for ${job.title}`} disabled={submitting} onClick={() => onLifecycle(item, 'warranty_claim', warrantyAction.warrantyClaimId)}><ShieldCheck size={16} />Warranty review</button> : null}
          {aftercareAction ? <button className="secondary-button" aria-label={`Complete aftercare for ${job.title}`} disabled={submitting} onClick={() => onLifecycle(item, 'aftercare', aftercareAction.aftercareId)}><BadgeCheck size={16} />Complete aftercare</button> : null}
          {canDraftRecurring ? <button className="secondary-button" aria-label={`Draft recurring plan for ${job.title}`} disabled={submitting} onClick={() => onDraftRecurring(item)}><RefreshCw size={16} />Service plan</button> : null}
          <button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => onOpen(job)}><ArrowUpRight size={16} /></button>
        </div>
      </article>
    })}{!rows.length ? <Empty title="No client work" detail="Client decisions, closeout, warranty, and aftercare records will appear here when action is needed." /> : null}</div>
  </section>
}

function FieldAssuranceWorkspace({ field, jobs, canCoordinate, canApprove, submitting, onPrepareSafety, onReview, onCapture, onOpenApprovals, onOpen }) {
  const rows = field?.jobs || field?.rows || EMPTY_LIST
  const summary = field?.summary || {}
  const targetFor = (item, action) => {
    const targets = {
      resolve_incident: { type: 'incident', recordId: action.incidentId, record: item.latest?.incident, label: 'Resolve incident', status: 'resolved' },
      resolve_observation: { type: 'observation', recordId: action.observationId, record: item.latest?.observation, label: 'Resolve observation', status: 'resolved' },
      review_rfi: { type: 'rfi', recordId: action.rfiId, record: item.latest?.rfi, label: 'Answer RFI', status: 'answered' },
      review_submittal: { type: 'submittal', recordId: action.submittalId, record: item.latest?.submittal, label: 'Approve submittal', status: 'approved' },
      review_permit: { type: 'permit', recordId: action.permitId, record: item.latest?.permit, label: 'Submit permit review', status: 'submitted' },
      review_document: { type: 'document', recordId: action.documentId, record: item.latest?.document, label: 'Approve document review', status: 'approved' },
      review_inspection: { type: 'inspection', recordId: action.inspectionId, record: item.latest?.inspection, label: 'Request inspection sign-off', status: 'passed' },
      clear_site_access: { type: 'site_access', recordId: action.siteAccessId, record: item.latest?.siteAccess, label: 'Clear site access', status: 'cleared' },
      ...
      complete_safety_meeting: { type: 'safety_meeting', recordId: action.safetyMeetingId, record: item.latest?.safetyMeeting, label: 'Complete safety talk', status: 'completed' },
      complete_orientation: { type: 'orientation', recordId: action.orientationId, record: item.latest?.orientation, label: 'Complete orientation', status: 'completed' },
      complete_quality_review: { type: 'quality_check', recordId: action.qualityCheckId, record: item.latest?.qualityCheck, label: 'Complete quality review', status: 'passed' },
      complete_safety_check: { type: 'safety_check', recordId: action.safetyCheckId, record: item.latest?.safetyCheck, label: 'Complete safety check', status: 'completed' },
      resolve_punch_item: { type: 'punch_item', recordId: action.punchItemId, record: item.latest?.punchItem, label: 'Resolve punch item', status: 'resolved' }
    }
    return targets[action.type]
  }

  return <div className="field-assurance-workspace" data-testid="field-assurance-workspace">
    <div className="assurance-heading"><div><h2>Assurance queue</h2><p>Safety, design, permit, inspection, quality, and evidence controls ranked from the retained ledger.</p></div><span className="count-badge">{rows.length}</span></div>
    <div className="assurance-summary" aria-label="Field assurance summary">
      <div><span>Incident blocked</span><strong>{summary.incidentBlocked || 0}</strong></div>
      <div><span>Design review</span><strong>{summary.designReviews || 0}</strong></div>
      <div><span>Quality review</span><strong>{summary.qualityReviews || 0}</strong></div>
      <div><span>Evidence missing</span><strong>{summary.evidenceMissing || 0}</strong></div>
    </div>
    <div className="assurance-list">{rows.map(item => {
      const job = jobs.find(candidate => candidate.id === item.jobId) || { id: item.jobId, title: item.jobTitle || 'Ledger job' }
      const canAct = canCoordinate && !item.flags?.approvalRequired
      const safetyAction = canAct ? item.nextActions?.find(action => action.type === 'prepare_safety_pack') : null
      const accessPrerequisiteAction = canAct && item.latest?.siteAccess?.orientationValid === false
        ? item.nextActions?.find(action => action.type === 'complete_orientation')
        : null
      const reviewAction = canAct
        ? accessPrerequisiteAction || FIELD_ASSURANCE_REVIEW_PRIORITY
          .map(type => item.nextActions?.find(action => action.type === type))
          .find(Boolean)
        : null
      ...
      return <article className="assurance-item" key={item.jobId}>
        <div className="assurance-copy">
          <div className="assurance-title"><h3>{item.jobTitle || 'Ledger job'}</h3><span className={`status status-${item.fieldStatus}`}>{formatStatus(item.fieldStatus)}</span></div>
          <p>{item.nextAction || 'Field assurance records are stable.'}</p>
          <div className="assurance-values">
            <span>Incidents <strong>{item.counts?.openIncidents || 0}</strong></span>
            <span>Design controls <strong>{(item.counts?.openRfis || 0) + (item.counts?.submittalReviews || 0) + (item.counts?.permitReviews || 0) + (item.counts?.documentReviews || 0)}</strong></span>
            <span>Quality controls <strong>{(item.counts?.inspectionReviews || 0) + (item.counts?.openObservations || 0) + (item.counts?.qualityOpen || 0) + (item.counts?.punchOpen || 0)}</strong></span>
            <span>Evidence <strong>{item.counts?.evidenceRecords || 0}</strong></span>
          </div>
          <div className="assurance-flags">
            ...
            {item.counts?.siteAccessBlocks ? <span className="tag">{item.counts.siteAccessBlocks} access block{item.counts.siteAccessBlocks === 1 ? '' : 's'}</span> : null}
          </div>
        </div>
        <div className="assurance-actions">
          {item.flags?.approvalRequired && canApprove ? <button className="secondary-button" disabled={submitting} onClick={() => onOpenApprovals({ jobId: item.jobId, jobTitle: job.title, approvalId: item.nextActions?.find(action => action.approvalId)?.approvalId || null })}><ShieldCheck size={16} />Review approval</button> : null}
          {safetyAction ? <button className="secondary-button" aria-label={`Prepare safety pack for ${job.title}`} disabled={submitting} onClick={() => onPrepareSafety(item)}><PackageCheck size={16} />Safety pack</button> : null}
          {reviewTarget?.recordId ? <button className="secondary-button" aria-label={`${reviewTarget.label} for ${job.title}`} disabled={submitting} onClick={() => onReview(item, reviewTarget)}><ClipboardCheck size={16} />{reviewTarget.label}</button> : null}
          {captureAction ? <button className="secondary-button" aria-label={`Capture field evidence for ${job.title}`} disabled={submitting} onClick={() => onCapture(item)}><FileUp size={16} />Capture evidence</button> : null}
          <button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => onOpen(job)}><ArrowUpRight size={16} /></button>
        </div>
      </article>
    })}{!rows.length ? <Empty title="Assurance queue is clear" detail="Safety, design, quality, and evidence controls will appear here when review is required." /> : null}</div>
  </div>
}

function AutomationControl({ commandPlan, scheduler, jobs, view, selectedIds, submitting, onViewChange, onToggle, onSelectVisible, onApply, onRun, onOpenApprovals, onOpen }) {
  ...
  return <section className="panel page-panel automation-panel" data-testid="automation-control">
    <div className="panel-heading automation-heading">
      <div><h2>Automation control</h2><p>Prioritized ledger work, retained drafts, and durable scheduler outcomes.</p></div>
      <span className={`status ${schedulerJob?.status === 'running' ? 'status-attention' : 'status-ready'}`}>{schedulerJob?.status || 'idle'}</span>
    </div>
    <div className="automation-summary" aria-label="Automation summary">
      <div><span>Safe drafts</span><strong>{summary.safeDraftable || 0}</strong></div>
      <div><span>Approval gates</span><strong>{summary.approvalRequired || 0}</strong></div>
      <div><span>Blocked</span><strong>{summary.blocked || 0}</strong></div>
      <div><span>External commitments</span><strong>{summary.externalCommitments || 0}</strong></div>
    </div>
    <div className="automation-toolbar">
      <div className="automation-tabs" role="tablist" aria-label="Automation action view">
        {[
          ...
          ['approval', 'Approvals'],
          ['blocked', 'Blocked']
        ].map(([key, label]) => <button key={key} role="tab" aria-selected={view === key} className={view === key ? 'automation-tab-active' : ''} onClick={() => onViewChange(key)}>{label}</button>)}
      </div>
      <div className="automation-commands">
        <button className="secondary-button" disabled={submitting || !visibleSafeIds.length} onClick={() => onSelectVisible(allVisibleSafeSelected ? [] : visibleSafeIds)}>{allVisibleSafeSelected ? <X size={15} /> : <Check size={15} />}{allVisibleSafeSelected ? 'Clear visible' : 'Select safe'}</button>
        <button className="primary-button" disabled={submitting || !selectedIds.length} onClick={onApply}><ClipboardCheck size={16} />{selectedIds.length ? `Apply ${selectedIds.length} draft${selectedIds.length === 1 ? '' : 's'}` : 'Apply selected'}</button>
        <button className="secondary-button" disabled={submitting || schedulerJob?.status === 'running'} onClick={onRun}><Activity size={16} />Run due cycle</button>
      </div>
    </div>
    {schedulerJob ? <div className="automation-run-state"><span>Last completed</span><strong>{formatDate(schedulerJob.lastCompletedAt)}</strong><span>{schedulerJob.lastResult?.actionCount || 0} action(s), {schedulerJob.lastResult?.blockedCount || 0} blocked</span></div> : null}
    <div className="automation-list">
      {actions.map(action => {
        const job = jobs.find(candidate => candidate.id === action.jobId) || (action.jobId ? { id: action.jobId, title: action.jobTitle || 'Ledger job' } : null)
        const selectable = action.safeDraftable && !action.blocked
        return <article className="automation-item" key={action.id}>
          <label className="automation-select">
            <input type="checkbox" disabled={!selectable || submitting} checked={selectable && selected.has(action.id)} onChange={event => onToggle(action.id, event.target.checked)} />
            <span className="visually-hidden">Select {action.message}</span>
          </label>
          <span className={`severity severity-${action.severity || 'medium'}`}><Activity size={14} /></span>
          <div className="automation-copy"><strong>{action.jobTitle || formatStatus(action.actionType)}</strong><p>{action.message}</p><small>{formatStatus(action.stream)} · {formatStatus(action.actionType)}</small></div>
          <div className="automation-flags">{action.safeDraftable ? <span className="tag tag-green">Safe draft</span> : null}{action.requiresApproval ? <span className="tag tag-amber">Approval</span> : null}{action.blocked ? <span className="tag">Blocked</span> : null}</div>
          {action.approvalId ? <button className="icon-button table-action" aria-label={`Review approval for ${action.jobTitle || 'ledger action'}`} onClick={() => onOpenApprovals({ jobId: action.jobId, jobTitle: action.jobTitle, approvalId: action.approvalId })}><ShieldCheck size={16} /></button> : job ? <button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => onOpen(job)}><ArrowUpRight size={16} /></button> : <span className="automation-action-spacer" />}
        </article>
      })}
      {!actions.length ? <Empty title="No matching automation work" detail="The selected command-plan view has no retained action waiting." /> : null}
    </div>
  </section>
}
...
function AuditHistory({ totalEvents = 0 }) {
  ...
  const facetOptions = (items, label) => <>
    ...
    {(items || []).map(item => <option key={item.value} value={item.value}>{formatStatus(item.value)} ({item.count})</option>)}
  </>

  return <section className="panel page-panel audit-history-panel" data-testid="audit-history-panel" aria-busy={auditLoading || undefined}>
    <div className="panel-heading audit-history-heading">
      <div><h2>Audit history</h2><p>Inspect retained operator, automation, approval, and lifecycle evidence in chain order.</p></div>
      <div className="audit-history-heading-actions"><span className="count-badge">{events.length} loaded</span><button className="icon-button" aria-label="Refresh audit history" disabled={auditLoading} onClick={() => loadHistory(appliedFilters, { includeFacets: true })}><RefreshCw size={16} className={auditLoading ? 'spin' : ''} /></button></div>
    </div>
    <form className="audit-filter-toolbar" data-testid="audit-history-filters" onSubmit={applyFilters}>
      <label className="search-control audit-search"><Search size={16} /><span className="visually-hidden">Search audit history</span><input type="search" maxLength="120" value={filters.query} onChange={event => setFilters({ ...filters, query: event.target.value })} placeholder="Search action, record, job, or actor" /></label>
      <label><span>Record type</span><select value={filters.entityType} onChange={event => setFilters({ ...filters, entityType: event.target.value })}>{facetOptions(facets.entityTypes, 'record types')}</select></label>
      <label><span>Action</span><select value={filters.action} onChange={event => setFilters({ ...filters, action: event.target.value })}>{facetOptions(facets.actions, 'actions')}</select></label>
      <label><span>Actor</span><select value={filters.actor} onChange={event => setFilters({ ...filters, actor: event.target.value })}>{facetOptions(facets.actors, 'actors')}</select></label>
      <label><span>From</span><input type="date" value={filters.from} max={filters.until || undefined} onChange={event => setFilters({ ...filters, from: event.target.value })} /></label>
      <label><span>Until</span><input type="date" value={filters.until} min={filters.from || undefined} onChange={event => setFilters({ ...filters, until: event.target.value })} /></label>
      <div className="audit-filter-actions"><button type="button" className="secondary-button" disabled={auditLoading || (!activeFilterCount && !Object.values(filters).some(Boolean))} onClick={clearFilters}><X size={15} />Clear</button><button className="primary-button" disabled={auditLoading}><Search size={15} />Apply</button></div>
    </form>
    <div className="audit-history-summary" aria-live="polite"><span>{totalEvents} chained event{totalEvents === 1 ? '' : 's'} retained</span><strong>{activeFilterCount ? `${activeFilterCount} active filter${activeFilterCount === 1 ? '' : 's'}` : 'Latest chain activity'}</strong>{page?.newestSequence ? <code>#{page.newestSequence} to #{page.oldestSequence}</code> : null}</div>
    {auditError ? <div className="audit-history-error" role="alert"><TriangleAlert size={16} /><span>{auditError}</span><button className="secondary-button" onClick={() => loadHistory(appliedFilters, { includeFacets: true })}>Retry</button></div> : null}
    <div className="audit-history-list" role="list">
      {events.map(event => <article className="audit-history-row" role="listitem" key={event.id}>
        <div className="audit-sequence"><span>Sequence</span><strong>#{event.sequenceNumber}</strong><code title={event.eventHash}>{shortHash(event.eventHash)}</code></div>
        <div className="audit-event-copy"><div><strong>{formatStatus(event.action)}</strong><span className="tag tag-green">Chained</span></div><p>{formatStatus(event.entityType)} / {event.entityId}</p>{event.jobId ? <small>Job {event.jobId}</small> : <small>Portfolio record</small>}</div>
        <div className="audit-event-context"><strong>{event.actor}</strong><span>{formatDateTime(event.createdAt)}</span></div>
        <button className="icon-button table-action" aria-label={`Inspect audit event ${event.sequenceNumber}`} onClick={clickEvent => { detailOpenerRef.current = clickEvent.currentTarget; setSelectedEvent(event) }}><Eye size={16} /></button>
      </article>)}
      {!events.length && !auditLoading && !auditError ? <Empty title="No audit events match" detail="Adjust the filters or refresh to inspect the latest retained chain activity." /> : null}
      {auditLoading && !events.length ? <div className="audit-history-loading"><LoaderCircle className="spin" size={22} /><span>Loading retained audit history</span></div> : null}
    </div>
    {page?.hasMore ? <div className="audit-history-more"><button className="secondary-button" disabled={auditLoading} onClick={() => loadHistory(appliedFilters, { append: true, beforeSequence: page.nextBeforeSequence })}>{auditLoading ? <LoaderCircle className="spin" size={15} /> : <ChevronRight size={15} />}Load older events</button></div> : null}

    {selectedEvent ? <div className="modal-backdrop audit-detail-backdrop" role="presentation">
      <section className="modal audit-detail-modal" role="dialog" aria-modal="true" aria-labelledby="audit-detail-title" data-testid="audit-event-detail">
        <div className="modal-heading"><div><p className="eyebrow">Chained event #{selectedEvent.sequenceNumber}</p><h2 id="audit-detail-title">{formatStatus(selectedEvent.action)}</h2><p>{formatDateTime(selectedEvent.createdAt)} / {selectedEvent.actor}</p></div><button ref={detailCloseRef} className="icon-button" aria-label="Close audit event detail" onClick={closeAuditDetail}><X size={18} /></button></div>
        <div className="audit-detail-body">
          <dl className="audit-detail-facts"><div><dt>Record type</dt><dd>{formatStatus(selectedEvent.entityType)}</dd></div><div><dt>Record id</dt><dd>{selectedEvent.entityId}</dd></div><div><dt>Job</dt><dd>{selectedEvent.jobId || 'Portfolio record'}</dd></div><div><dt>Event id</dt><dd>{selectedEvent.id}</dd></div></dl>
          <div className="audit-chain-proof"><div><span>Previous hash</span><code>{selectedEvent.previousHash}</code></div><ChevronRight size={18} /><div><span>Event hash</span><code>{selectedEvent.eventHash}</code></div></div>
          <div className="audit-payload-grid">
            <section><h3>Before</h3><pre>{JSON.stringify(selectedEvent.before ?? null, null, 2)}</pre></section>
            <section><h3>After</h3><pre>{JSON.stringify(selectedEvent.after ?? null, null, 2)}</pre></section>
            <section><h3>Metadata</h3><pre>{JSON.stringify(selectedEvent.metadata ?? null, null, 2)}</pre></section>
          </div>
        </div>
        <div className="modal-actions"><button className="secondary-button" onClick={closeAuditDetail}>Close</button></div>
      </section>
    </div> : null}
  </section>
}
...
function AuthenticationScreen({ checking, error, submitting, onLogin }) {
  ...
  return <main className="auth-shell">
    <section className="auth-panel" aria-busy={checking || submitting}>
      <div className="auth-brand"><span className="brand-mark"><HardHat size={18} /></span><strong>Contractor.AI</strong></div>
      {checking ? <div className="auth-loading"><LoaderCircle className="spin" size={25} /><span>Checking operator access</span></div> : <>
        <div className="auth-heading"><span className="auth-icon"><LockKeyhole size={21} /></span><div><h1>Operator sign in</h1><p>Use the access key issued for your role.</p></div></div>
        <form className="auth-form" onSubmit={submit}>
          <label htmlFor="operator-access-key">Access key</label>
          <div className="auth-input-wrap">
            <input id="operator-access-key" autoComplete="current-password" autoFocus minLength="32" required type={visible ? 'text' : 'password'} value={token} onChange={event => setToken(event.target.value)} />
            <button type="button" className="icon-button" aria-label={visible ? 'Hide access key' : 'Show access key'} onClick={() => setVisible(current => !current)}>{visible ? <EyeOff size={17} /> : <Eye size={17} />}</button>
          </div>
          {error ? <div className="auth-error" role="alert"><TriangleAlert size={16} /><span>{error}</span></div> : null}
          <button className="primary-button auth-submit" disabled={submitting || token.length < 32}><LockKeyhole size={16} />{submitting ? 'Signing in...' : 'Sign in'}</button>
        </form>
      </>}
    </section>
  </main>
...
function App() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [jobsResult, healthResult, readinessResult] = await Promise.all([
        api('/api/ledger/jobs?limit=100'), api('/api/health'), api('/api/readiness').catch(() => null)
      ])
      if (fieldScoped) {
  ...
  const loginDefenseLabel = loginRateLimitCapability?.durability === 'ledger' && loginRateLimitCapability?.multiReplicaSafe
    ? `${loginRateLimitCapability.limit} failures / ${Math.round(loginRateLimitCapability.windowMs / 60_000)} min, durable`
    : operationCapabilities ? 'attention' : 'checking'
  const apiRateLimitCapability = operationCapabilities?.requestSafety?.apiRateLimit
  const apiDefenseLabel = apiRateLimitCapability?.durability === 'ledger' && apiRateLimitCapability?.multiReplicaSafe
  ...
  const auditIntegrityLabel = auditIntegrityCapability?.valid && auditIntegrityCapability?.appendMode === 'atomic_hash_chain'
  ...
  const activeJobs = useMemo(() => jobs.filter(job => !['archived', 'completed', 'cancelled', 'rejected'].includes(job.status)).slice(0, 8), [jobs])
  const taskAssigneeOptions = Array.from(new Map(
    (selectedJob?.assignments || [])
      .filter(assignment => assignment.workerId && !['released', 'cancelled', 'completed', 'closed', 'rejected'].includes(assignment.status))
  ...
  const syncFieldOutbox = useCallback(async ({ announce = false } = {}) => {
    ...
    try {
      ...
      if (announce && result.stopped && result.stopped !== 'offline') setError(result.stopped.message || 'A queued field update could not be recorded.')
    } catch (requestError) {
  ...
  useEffect(() => {
    ...
    initializeOutbox()
    const handleOnline = () => { syncFieldOutbox() }
  ...
  async function resolveApproval(event) {
    ...
    setSubmitting(true)
    try {
      await api(`/api/ledger/approvals/${item.id}/resolve`, { method: 'POST', body: JSON.stringify({ status, resolvedBy: 'Owner', reason: approvalReason.trim() || `Reviewed ${item.summary || item.targetType} and its retained safeguards.` }) })
      setApprovalReview(null)
      setApprovalReason('')
  ...
  async function runCycle() {
    setSubmitting(true)
    try {
      const result = await api('/api/ledger/scheduler/run', { method: 'POST', body: JSON.stringify({ actor: 'owner_scheduler', maxActions: 10 }) })
      if (result.ran) {
        notify(`Durable cycle completed with ${result.result?.applied?.length || 0} internal draft action(s) and ${result.result?.blocked?.length || 0} blocked action(s). No external commitment was made.`)
      } else {
  ...
  function toggleCommandSelection(actionId, checked) {
    setSelectedCommandIds(current => checked
  ...
  async function applySelectedCommands() {
    ...
    try {
      ...
      notify(`${result.summary?.applied || 0} safe command-plan draft(s) retained; ${result.summary?.skipped || 0} action(s) skipped. External commitments remain zero.`)
      await refresh()
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  async function backup() {
    ...
    try {
      ...
      notify(`Local backup created with ${result.backup.verification.checkedFiles} checksummed file(s), including ${result.backup.evidenceFiles || 0} evidence file(s).`)
      await refresh()
    } catch (requestError) { setError(requestError.message) } finally { setSubmitting(false) }
  }
  ...
  async function validateExport(file) {
    ...
    try {
      ...
      notify(`Export checksum verified: ${result.counts.jobs} jobs and ${result.counts.approvals} approvals. This artifact is for reconciliation, not restore.`)
    } catch (requestError) {
  ...
  async function uploadEvidence(event) {
    ...
    const draft = { id: createFieldEvidenceDraftId(), jobId: evidence.jobId, notes: evidence.notes, riskLevel: evidence.riskLevel, file, operatorScope: outboxScope }
    setSubmitting(true)
    try {
  ...
  async function recordFieldProgress(event) {
    ...
    if (!fieldProgress.jobId || !Number.isFinite(progressPercent) || progressPercent < 0 || progressPercent > 100 || !fieldProgress.note.trim()) {
    ...
    try {
      ...
      setFieldProgress(emptyFieldProgress())
      notify(result.progress?.replayed
  ...
  async function recordFieldDailyLog(event) {
    ...
    if (!fieldDailyLog.jobId || !fieldDailyLog.workDate || !(hours > 0 && hours <= 24) || !(manpower > 0 && manpower <= 500) || !fieldDailyLog.workCompleted.trim()) {
    ...
    try {
      ...
      setFieldDailyLog(emptyFieldDailyLog())
      notify(result.dailyLog?.replayed
  ...
  async function loadResourceOptions() {
    try {
      const [workersResult, toolsResult] = await Promise.all([
        api('/api/ledger/workers?limit=100'),
        api('/api/ledger/tools?limit=100')
      ])
      setResourceOptions({
        workers: (workersResult.workers || []).filter(worker => !['retired', 'inactive'].includes(worker.status)),
        tools: (toolsResult.tools || []).filter(tool => (
          tool.status === 'available'
          && !tool.retirementApprovalId
          && tool.inspection?.reservationReady !== false
        ))
      })
    } catch (requestError) {
      setError(requestError.message)
  ...
  async function openJobWorkspace(job) {
    ...
    setWeatherDraft({ condition: 'workable', precipitationPercent: '0', weatherSensitive: /garden|pav|roof|fence|outside|outdoor|painting|clean/i.test(`${job.jobType || ''} ${job.title || ''} ${job.description || ''}`) })
  ...
  async function assignWorker(event) {
    ...
    try {
      ...
      notify(result.assignment.requiresApproval
  ...
  async function reserveTool(event) {
    ...
    try {
      ...
      notify(result.toolReservation.requiresApproval
  ...
  async function createCommunicationDraft(event) {
    ...
    try {
      ...
      notify(`Client ${communicationDraft.channel} draft created. Approval ${result.approval?.id ? 'is required before delivery' : 'status was recorded'}.`)
      await Promise.all([refresh(), loadSelectedJob(selectedJobId)])
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  async function saveWorker(event) {
    ...
    try {
      ...
      const result = await api(editing
        ? `/api/ledger/workers/${encodeURIComponent(workerDraft.id)}`
        : '/api/ledger/workers', {
        ...
        body: JSON.stringify({
          ...
          skills: workerDraft.skills.split(',').map(item => item.trim()).filter(Boolean),
      ...
      setWorkerEditor(null)
      setWorkerDraft(emptyWorkerDraft())
      notify(`${result.worker.name} retained as ${formatStatus(result.worker.status)}. No schedule, payroll, or contact action was created.`)
      await refresh()
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  async function saveEquipment(event) {
    ...
    try {
      ...
      const result = await api(editing
        ? `/api/ledger/tools/${encodeURIComponent(equipmentDraft.id)}`
        : '/api/ledger/tools', {
        method: editing ? 'PUT' : 'POST',
        body: JSON.stringify({
  ...
  async function recordEquipmentInspection(event) {
    ...
    try {
      ...
      setEquipmentInspection(null)
      setEquipmentInspectionDraft(emptyEquipmentInspectionDraft())
      notify(`${result.tool.name} inspection retained as ${formatStatus(result.inspection.result)}. ${result.reservationReady ? 'Inspection readiness is clear.' : 'New reservations remain blocked.'}`)
      await refresh()
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  async function recordEquipmentMaintenance(event) {
    event.preventDefault()
    if (!equipmentMaintenance || equipmentMaintenanceDraft.performedBy.trim().length < 2 || equipmentMaintenanceDraft.notes.trim().length < 8) return
    setSubmitting(true)
    try {
  ...
  async function saveTradePartner(event) {
    ...
    try {
      ...
      const result = await api(editing
        ? `/api/ledger/trade-partners/${encodeURIComponent(tradePartnerDraft.id)}`
        : '/api/ledger/trade-partners', {
        method: editing ? 'PUT' : 'POST',
        body: JSON.stringify({
          ...tradePartnerDraft,
          ...
          specialties: tradePartnerDraft.specialties.split(',').map(item => item.trim()).filter(Boolean),
  ...
  async function draftProcurementOrder(item) {
    ...
    const tradePartner = tradePartners.find(partner => partner.status === 'active' && partner.name.toLowerCase() === supplierName.toLowerCase())
    setSubmitting(true)
    try {
      ...
      notify(`Procurement draft retained for ${currency.format(result.procurementOrder?.amount || 0)}. Approval is required before supplier commitment.`)
      await refresh()
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  function openResourceControl(item, action) {
    ...
    const material = item.materials?.find(candidate => candidate.id === action.materialRequirementId)
      || item.latest?.material
      || null
    const workerName = action.workerName
      || item.latest?.assignment?.workerName
      || item.workers?.[0]?.name
      || ''
    const procurementOrder = action.recordType === 'procurement_order' ? action.record : null
    const matchedPartner = tradePartners.find(partner => partner.id === procurementOrder?.tradePartnerId)
  ...
  async function submitResourceControl(event) {
    ...
    if (type === 'complete_worker_orientation') {
      ...
      if (!procurementOrderId || !resourceActionDraft.tradePartnerId || !resourceActionDraft.supplier.trim() || !(amount > 0) || !resourceActionDraft.requiredBy) {
    ...
    try {
      ...
      if (type === 'complete_worker_orientation') {
        ...
        notify(`Procurement approval requested for ${currency.format(result.procurementOrder?.amount || 0)}. No supplier order or spend commitment was made.`)
      } else if (type === 'record_time_log') {
        notify('Worker time was recorded in the retained job ledger.')
      } else {
        notify(`${formatStatus(result.materialRequirement?.status)} material evidence retained. No supplier order or spend commitment was made.`)
      }
      closeResourceControl()
      await refresh()
    } catch (requestError) {
  ...
  async function openInvoiceDraft(item, action = null) {
    ...
    try {
      ...
      const milestone = action?.billingMilestoneId
        ...
        : null
      const amount = Math.max(0, Number(milestone?.amount ?? action?.amount ?? item.money?.invoiceDraftAmount ?? quote?.subtotal ?? item.money?.contractValue ?? 0))
  ...
  async function prepareInvoicePackage(item, action) {
    ...
    try {
      ...
      notify(`${creditNotePackage ? 'Credit note' : 'Invoice'} ${result.issueReference} retained with ${result.structuredExportIncluded ? 'HTML and UBL attachments' : 'a human-readable attachment'}. ${creditNotePackage ? 'The receivable was adjusted; delivery' : 'Delivery'} still requires approval and a verified receipt.`)
      await refresh()
    } catch (requestError) {
      setError(requestError.message)
    } finally {
  ...
  function openFinanceControl(item, action) {
    ...
    setFinanceActionDraft({
      ...
      description: action.type === 'create_billing_milestone'
  ...
  async function submitFinanceControl(event) {
    ...
    if (type === 'create_credit_note') {
      ...
      if (!(financeControlHours > 0 || financeControlExpense > 0) || financeControlHours > 24 || financeControlRate < 0 || financeControlExpense < 0) {
      ...
      body = {
        ...
        expense: financeControlExpense > 0 ? {
      ...
      if (!(financeControlAmount > 0) || financeControlForecast < 0 || !financeActionDraft.costCode.trim() || !financeActionDraft.description.trim()) {
      ...
      if (!(financeControlAmount > 0) || !Number.isFinite(taxRate) || taxRate < 0 || taxRate > 100
        || !financeActionDraft.description.trim() || !financeActionDraft.plannedIssueAt || !financeActionDraft.dueAt) {
      ...
      if (!(financeControlAmount > 0) || !financeActionDraft.dueAt || !Number.isFinite(percentComplete) || percentComplete < 0 || percentComplete > 100) {
    ...
    try {
      ...
      if (type === 'create_credit_note') {
        ...
        notify(`Billing milestone retained for ${currency.format(result.billingMilestone?.total || financeControlAmount)}. Approval is required before an invoice can be derived.`)
      } else if (type === 'request_lien_waiver') {
  ...
  async function draftClientFollowup(item) {
    ...
    const subject = selection?.title
      ? `Selection reminder: ${selection.title}`
      : `Client follow-up: ${item.jobTitle || 'project update'}`
  ...
  async function submitClientLifecycle(event) {
    ...
    setSubmitting(true)
    try {
      const result = await api(`/api/ledger/jobs/${encodeURIComponent(clientAction.item.jobId)}/lifecycle/${encodeURIComponent(clientAction.type)}/${encodeURIComponent(clientAction.recordId)}`, {
        method: 'PATCH',
        body: JSON.stringify(payload)
      })
  ...
  async function submitFieldReview(event) {
    ...
    setSubmitting(true)
    try {
      const result = await api(`/api/ledger/jobs/${encodeURIComponent(fieldAction.item.jobId)}/lifecycle/${encodeURIComponent(fieldAction.type)}/${encodeURIComponent(fieldAction.recordId)}`, {
        method: 'PATCH',
        body: JSON.stringify(payload)
      })
      notify(result.approvalRequired
  ...
  async function resetQa() {
    ...
    setSubmitting(true)
    try {
      const result = await api('/api/operations/reset-qa', { method: 'POST', body: JSON.stringify({ confirmation: 'RESET_QA', actor: 'office_operator' }) })
  ...
  if (authState === 'checking' || authState === 'required') {
    return <AuthenticationScreen checking={authState === 'checking'} error={authError} submitting={authSubmitting} onLogin={loginOperator} />
  }
  ...
  return <div className="app-shell">
    <aside className={`side-nav ${mobileNavOpen ? 'side-nav-open' : ''}`} aria-label="Primary navigation">
      <div className="brand"><span className="brand-mark"><HardHat size={18} /></span><span>Contractor.AI</span></div>
      <div className="nav-list">{visibleNavItems.map(([key, label, icon]) => <button key={key} className={section === key ? 'nav-active' : ''} onClick={() => selectSection(key)}>{createElement(icon, { size: 18 })}{label}{key === 'approvals' && metrics.pendingApprovals > 0 ? <b>{metrics.pendingApprovals}</b> : null}</button>)}</div>
      <div className="nav-footer"><CloudOff size={16} /><span>Local-first ledger</span><small>External actions require approval</small></div>
    </aside>

    <main className="workspace" aria-busy={loading}>
      <header className="topbar">
        <button className="icon-button mobile-only" aria-label="Open navigation" onClick={() => setMobileNavOpen(true)}><Menu size={20} /></button>
        <div><h1>{pageTitle}</h1><p>{dashboard ? `${metrics.openJobs || 0} active jobs, ${metrics.pendingApprovals || 0} decisions awaiting review` : 'Loading the local operating ledger'}</p></div>
        <div className="topbar-actions"><span className="sync-state"><CloudOff size={15} />{fieldScoped ? 'Field scope' : 'Local-first'}</span>{operator.authenticated ? <span className="operator-session" title={formatStatus(operator.role)}><ShieldCheck size={14} /><span>{operator.name || formatStatus(operator.role)}</span></span> : null}<button className="icon-button" aria-label="Refresh data" onClick={refresh} disabled={loading}><RefreshCw size={18} className={loading ? 'spin' : ''} /></button>{operator.authenticated ? <button className="icon-button" aria-label="Sign out" title="Sign out" onClick={logoutOperator} disabled={submitting}><LogOut size={17} /></button> : null}{capabilities.intake ? <button className="primary-button" onClick={() => setShowIntake(true)}><Plus size={17} />New intake</button> : null}</div>
      </header>

      {notice ? <div className="notice"><Check size={16} />{notice.message}<button aria-label="Dismiss notice" onClick={() => setNotice(null)}><X size={15} /></button></div> : null}
      {error ? <div className="error-banner"><TriangleAlert size={18} /><span>{error}</span><button onClick={refresh}>Retry</button></div> : null}

      {loading && !data ? <div className="loading"><LoaderCircle className="spin" size={26} />Loading the operating ledger</div> : null}
      {data ? <>
        {section === 'today' && <section className="page-grid">
          <div className="metrics-grid">
            <Metric icon={BriefcaseBusiness} label="Jobs today" value={metrics.openJobs || 0} hint={`${metrics.completedJobs || 0} completed`} />
            <Metric icon={MapPin} label="Dispatch ready" value={metrics.dispatchReadyJobs || 0} hint="Mobilize when cleared" tone="blue" />
            <Metric icon={ClipboardCheck} label="Approvals" value={metrics.pendingApprovals || 0} hint="Operator decision required" tone="amber" />
            <Metric icon={Gauge} label="Pipeline" value={currency.format(dashboard.money?.estimatedPipeline || 0)} hint="Estimated active value" tone="green" />
          </div>
          <div className="content-grid">
            <section className="panel command-panel"><div className="panel-heading"><div><h2>{fieldScoped ? 'My field work' : 'Command queue'}</h2><p>{fieldScoped ? 'Your assigned ledger jobs and retained field updates' : 'Ranked internal work and approval gates'}</p></div>{capabilities.approvals ? <button className="text-button" onClick={() => selectSection('approvals')}>Open approvals <ChevronRight size={15} /></button> : null}</div>
              {nextActions.length ? <div className="queue-list">{nextActions.map((item, index) => <div className="queue-row" key={`${item.type}-${item.jobId || index}`}><span className={`severity severity-${item.severity || 'medium'}`}><TriangleAlert size={15} /></span><div><strong>{item.title || formatStatus(item.type)}</strong><p>{item.message}</p></div>{item.requiresApproval ? <span className="tag tag-amber">Approval</span> : <span className="tag">Draft</span>}{item.jobId ? <button className="icon-button table-action" aria-label={`Open ${item.jobTitle || item.title || 'linked job'}`} onClick={() => openJobWorkspace(jobs.find(job => job.id === item.jobId) || { id: item.jobId, title: item.jobTitle || item.title || 'Ledger job' })}><ArrowUpRight size={16} /></button> : null}</div>)}</div> : <Empty title="Queue is clear" detail="No immediate ledger action needs review." />}
            </section>
            <section className="panel schedule-panel"><div className="panel-heading"><div><h2>Jobs schedule</h2><p>Live work from the ledger</p></div><button className="text-button" onClick={() => selectSection('jobs')}>All jobs <ChevronRight size={15} /></button></div>
              <div className="table-wrap"><table><thead><tr><th>When</th><th>Job</th><th>Location</th><th>Status</th><th aria-label="Open job" /></tr></thead><tbody>{activeJobs.slice(0, 6).map(job => <tr key={job.id}><td>{formatDate(job.scheduledStart || job.targetCompletion)}</td><td><strong>{job.title}</strong><small>{job.clientName || 'Client pending'}</small></td><td>{job.city || job.address || 'Location pending'}</td><td><span className={`status status-${job.status}`}>{formatStatus(job.status)}</span></td><td><button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => openJobWorkspace(job)}><ArrowUpRight size={16} /></button></td></tr>)}</tbody></table></div>
            </section>
          </div>
          <div className="bottom-grid">
            {capabilities.approvals ? <section className="panel compact-panel"><div className="panel-heading"><h2>Approval queue</h2><button className="text-button" onClick={() => selectSection('approvals')}>Review all <ChevronRight size={15} /></button></div>{data.approvals.slice(0, 4).map(item => <div className="compact-row" key={item.id}><ClipboardCheck size={16} /><div><strong>{item.title || formatStatus(item.targetType)}</strong><small>{item.jobTitle || item.summary || 'Ledger decision'}</small></div><span>{formatDate(item.createdAt)}</span></div>)}</section> : null}
            {capabilities.fieldEvidence ? <section className="panel compact-panel"><div className="panel-heading"><h2>Field evidence</h2><button className="text-button" onClick={() => selectSection('field')}>Open field <ChevronRight size={15} /></button></div><div className="field-summary"><PackageCheck size={26} /><div><strong>{metrics.storedDocuments || 0} stored records</strong><p>{metrics.fieldReports || 0} field reports, {metrics.openIncidents || 0} open incidents</p></div></div><button className="secondary-button full-button" onClick={() => selectSection('field')}><HardHat size={16} />Review field assurance</button></section> : null}
            {capabilities.maintenance ? <section className="panel compact-panel"><div className="panel-heading"><h2>Automation</h2><span className="tag tag-green">Ledger-only</span></div><p className="panel-copy">The durable cycle creates internal drafts and approval requests. It cannot send messages, commit spend, or confirm dates.</p><button className="secondary-button full-button" disabled={submitting} onClick={runCycle}><Activity size={16} />Run due cycle</button></section> : null}
          </div>
        </section>}

        {section === 'jobs' && <section className="panel page-panel"><div className="panel-heading"><div><h2>{fieldScoped ? 'My assigned jobs' : 'All jobs'}</h2><p>{fieldScoped ? 'Only jobs available to your field assignment.' : 'Persisted operational work, ordered by latest update.'}</p></div>{capabilities.intake ? <button className="primary-button" onClick={() => setShowIntake(true)}><Plus size={16} />New intake</button> : null}</div><div className="table-wrap"><table><thead><tr><th>Job</th><th>Client</th><th>Location</th>{!fieldScoped ? <th>Value</th> : null}<th>Progress</th><th>Status</th><th aria-label="Open job" /></tr></thead><tbody>{jobs.map(job => <tr key={job.id}><td><strong>{job.title}</strong><small>{job.jobType}</small></td><td>{job.clientName || 'Not set'}</td><td>{job.city || job.address || 'Not set'}</td>{!fieldScoped ? <td>{currency.format(job.contractValue || job.estimatedCost || 0)}</td> : null}<td><div className="progress"><span style={{ width: `${Math.min(100, job.progressPercent || 0)}%` }} /></div><small>{Math.round(job.progressPercent || 0)}%</small></td><td><span className={`status status-${job.status}`}>{formatStatus(job.status)}</span></td><td><button className="icon-button table-action" aria-label={`Open ${job.title}`} onClick={() => openJobWorkspace(job)}><ArrowUpRight size={16} /></button></td></tr>)}</tbody></table></div></section>}

        {section === 'approvals' && capabilities.approvals && <section className="panel page-panel">
          <div className="panel-heading"><div><h2>Approval queue</h2><p>{approvalFocus ? `Focused on the retained decision for ${approvalFocus.jobTitle || 'this job'}.` : 'Review the exact effects and safeguards before resolving a retained decision.'}</p></div>{approvalFocus ? <button className="text-button" onClick={() => setApprovalFocus(null)}>Show all approvals <span className="count-badge">{approvals.length}</span></button> : <span className="count-badge">{approvals.length}</span>}</div>
          {visibleApprovals.length ? <div className="approval-list">{visibleApprovals.map(item => <ApprovalQueueItem key={item.id} item={item} submitting={submitting} onReview={openApprovalReview} />)}</div> : <Empty title={approvalFocus ? 'Decision no longer waiting' : 'No approvals waiting'} detail={approvalFocus ? 'The focused decision was resolved or is no longer pending. Show all approvals to continue reviewing the queue.' : 'The current ledger has no pending consequential decision.'} />}
        </section>}

        {section === 'dispatch' && capabilities.dispatch ? <DispatchWorkspace dispatch={data.dispatch} jobs={jobs} canCoordinate={canCoordinate} canApprove={capabilities.approvals === true} submitting={submitting} onPrepare={prepareDispatchPack} onControl={openDispatchControl} onReviewWorkforce={reviewDispatchWorkforce} onReviewEquipment={reviewDispatchEquipment} onOpenApprovals={openApprovals} onOpen={openJobWorkspace} /> : null}
        ...
        {section === 'field' && capabilities.fieldEvidence && <section className="panel page-panel field-workspace" data-testid="field-workspace" aria-busy={loading || undefined}>
          ...
          {!fieldScoped ? <FieldAssuranceWorkspace field={data.field} jobs={jobs} canCoordinate={canCoordinate} canApprove={capabilities.approvals === true} submitting={submitting} onPrepareSafety={prepareFieldSafetyPack} onReview={openFieldReview} onCapture={focusFieldCapture} onOpenApprovals={openApprovals} onOpen={openJobWorkspace} /> : null}
          <div className="field-grid"><Metric icon={FolderArchive} label="Evidence stored" value={metrics.storedDocuments || 0} hint="Photos and documents" /><Metric icon={TriangleAlert} label="Open incidents" value={metrics.openIncidents || 0} hint="Require a review" tone="amber" /><Metric icon={ShieldCheck} label="Safety checks" value={metrics.safetyChecks || 0} hint="Ledger safety controls" tone="green" /></div>
          <div className="field-note"><HardHat size={20} /><div><strong>Field records remain local and auditable.</strong><p>Evidence, progress, time, and daily safety records use a bounded offline outbox. Exact retries are scoped to this operator and cannot create duplicate ledger entries.</p></div><div className="field-outbox-status" aria-live="polite">{outboxPending ? <span className="tag tag-amber">{outboxPending} queued</span> : <span className="tag tag-green">Outbox clear</span>}{outboxQuarantined ? <span className="tag">{outboxQuarantined} other scope</span> : null}</div></div>
          <form className="evidence-form daily-site-log" data-testid="daily-site-log-form" onSubmit={recordFieldDailyLog}>
            <div className="panel-heading"><div><h2>Daily site log</h2><p>Submit the site report, crew time card, and safety state as one retained ledger entry.</p></div>{fieldScoped && operator.worker?.name ? <span className="tag tag-green">{operator.worker.name}</span> : null}</div>
            <div className="form-grid">
              <label>Job<select required value={fieldDailyLog.jobId} onChange={event => setFieldDailyLog({ ...fieldDailyLog, jobId: event.target.value })}><option value="">Select an active job</option>{activeJobs.map(job => <option key={job.id} value={job.id}>{job.title}</option>)}</select></label>
              {!fieldScoped ? <label>Crew member<select required value={fieldDailyLog.workerId} onChange={event => setFieldDailyLog({ ...fieldDailyLog, workerId: event.target.value })}><option value="">Select a crew member</option>{workers.filter(worker => !['retired', 'inactive'].includes(worker.status)).map(worker => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select></label> : null}
              <label>Work date<input required type="date" value={fieldDailyLog.workDate} onChange={event => setFieldDailyLog({ ...fieldDailyLog, workDate: event.target.value })} /></label>
              <label>Hours worked<input required type="number" min="0.25" max="24" step="0.25" inputMode="decimal" value={fieldDailyLog.hours} onChange={event => setFieldDailyLog({ ...fieldDailyLog, hours: event.target.value })} /></label>
              <label>People on site<input required type="number" min="1" max="500" step="1" inputMode="numeric" value={fieldDailyLog.manpower} onChange={event => setFieldDailyLog({ ...fieldDailyLog, manpower: event.target.value })} /></label>
              <label>Weather<select value={fieldDailyLog.weather} onChange={event => setFieldDailyLog({ ...fieldDailyLog, weather: event.target.value })}><option value="clear">Clear</option><option value="cloudy">Cloudy</option><option value="rain">Rain</option><option value="wind">High wind</option><option value="heat">Heat</option><option value="cold">Cold</option></select></label>
              <label className="form-span">Work completed<textarea required minLength="3" value={fieldDailyLog.workCompleted} onChange={event => setFieldDailyLog({ ...fieldDailyLog, workCompleted: event.target.value })} placeholder="Record the work completed during this shift." /></label>
              <label className="form-span">Blockers or follow-up<textarea value={fieldDailyLog.blockers} onChange={event => setFieldDailyLog({ ...fieldDailyLog, blockers: event.target.value })} placeholder="One blocker or follow-up per line." /></label>
              <label className="form-span checkbox-label"><input type="checkbox" checked={fieldDailyLog.safetyConcern} onChange={event => setFieldDailyLog({ ...fieldDailyLog, safetyConcern: event.target.checked })} />A safety concern requires office review</label>
              {fieldDailyLog.safetyConcern ? <><label>Risk level<select value={fieldDailyLog.safetyRiskLevel} onChange={event => setFieldDailyLog({ ...fieldDailyLog, safetyRiskLevel: event.target.value })}><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></label><label className="form-span">Safety concern<textarea required minLength="5" value={fieldDailyLog.safetyNotes} onChange={event => setFieldDailyLog({ ...fieldDailyLog, safetyNotes: event.target.value })} placeholder="Describe the hazard, immediate control, and required follow-up." /></label></> : null}
            </div>
            <div className="modal-actions"><button className="primary-button" disabled={submitting}><ClipboardList size={16} />{submitting ? 'Submitting...' : navigator.onLine === false ? 'Save daily log offline' : 'Submit daily log'}</button></div>
          </form>
          <form ref={fieldCaptureRef} className="evidence-form" data-testid="field-evidence-form" onSubmit={uploadEvidence}>
            <div className="panel-heading"><div><h2>Record field evidence</h2><p>Photos, PDFs, and DOCX files remain behind authenticated ledger access.</p></div>{outboxPending ? <span className="tag tag-amber">{outboxPending} queued</span> : null}</div>
            <div className="form-grid">
              <label>Job<select required value={evidence.jobId} onChange={event => setEvidence({ ...evidence, jobId: event.target.value })}><option value="">Select an active job</option>{activeJobs.map(job => <option key={job.id} value={job.id}>{job.title}</option>)}</select></label>
              <label>Risk level<select value={evidence.riskLevel} onChange={event => setEvidence({ ...evidence, riskLevel: event.target.value })}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></label>
              <label className="form-span">Evidence file<input ref={evidenceInputRef} required type="file" accept="image/jpeg,image/png,image/webp,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document" /></label>
              <label className="form-span">Site note<textarea value={evidence.notes} onChange={event => setEvidence({ ...evidence, notes: event.target.value })} placeholder="Describe what this file proves, what changed, or what needs review." /></label>
            </div>
            <div className="modal-actions">{outboxPending ? <button type="button" className="secondary-button" disabled={outboxSyncing || submitting || navigator.onLine === false} onClick={() => syncFieldOutbox({ announce: true })}><RefreshCw size={16} className={outboxSyncing ? 'spin' : ''} />{outboxSyncing ? 'Syncing...' : `Sync ${outboxPending} queued`}</button> : null}<button className="primary-button" disabled={submitting}><FileUp size={16} />{submitting ? 'Recording...' : navigator.onLine === false ? 'Save offline draft' : 'Record evidence'}</button></div>
          </form>
          <form className="evidence-form" data-testid="field-progress-form" onSubmit={recordFieldProgress}>
            <div className="panel-heading"><div><h2>Record progress</h2><p>Field updates stay auditable. Completing a job remains an office-controlled decision.</p></div></div>
            <div className="form-grid">
              <label>Job<select required value={fieldProgress.jobId} onChange={event => setFieldProgress({ ...fieldProgress, jobId: event.target.value })}><option value="">Select an active job</option>{activeJobs.map(job => <option key={job.id} value={job.id}>{job.title}</option>)}</select></label>
              <label>Work state<select value={fieldProgress.status} onChange={event => setFieldProgress({ ...fieldProgress, status: event.target.value })}><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="on_hold">On hold</option></select></label>
              <label>Progress (%)<input required type="number" min="0" max="100" step="1" value={fieldProgress.progressPercent} onChange={event => setFieldProgress({ ...fieldProgress, progressPercent: event.target.value })} /></label>
              <label className="form-span">Field note<textarea required value={fieldProgress.note} onChange={event => setFieldProgress({ ...fieldProgress, note: event.target.value })} placeholder="Record completed work, blocker, or site condition for the office team." /></label>
            </div>
            <div className="modal-actions"><button className="primary-button" disabled={submitting}><Activity size={16} />{submitting ? 'Recording...' : navigator.onLine === false ? 'Save progress offline' : 'Record progress'}</button></div>
          </form>
        </section>}
        ...
        {section === 'operations' && capabilities.maintenance && <section className="operations-grid">
          ...
          <section className="panel page-panel">
            ...
            <div className="readiness-list"><div><span>Runtime</span><strong>{data.health.runtime?.mode || 'local'}</strong></div><div data-testid="storage-readiness"><span>Storage</span><strong>{evidenceStorageLabel}</strong></div><div><span>Authorization</span><strong>{data.health.runtime?.auth?.configuredRoles?.length ? data.health.runtime.auth.configuredRoles.join(', ') : data.health.runtime?.auth?.legacyOwnerTokenConfigured ? 'owner token' : 'local owner'}</strong></div><div data-testid="login-defense-readiness"><span>Login defense</span><strong>{loginDefenseLabel}</strong></div><div data-testid="api-defense-readiness"><span>API defense</span><strong>{apiDefenseLabel}</strong></div><div data-testid="audit-integrity-readiness"><span>Audit integrity</span><strong>{auditIntegrityLabel}</strong></div><div><span>Scheduler</span><strong>{data.scheduler?.enabled ? `every ${data.scheduler.intervalSeconds}s` : 'manual only'}</strong></div><div><span>Field retries</span><strong>{operationCapabilities?.requestSafety?.evidenceUploadIdempotency === 'durable' && operationCapabilities?.requestSafety?.progressEntryKey === 'durable' && operationCapabilities?.requestSafety?.dailyLogEntryKey === 'durable' ? 'scoped + deduplicated' : 'checking'}</strong></div><div><span>Backups</span><strong>{operationCapabilities?.backup?.portableDownload ? 'verified + portable' : providerRecovery?.available ? 'provider-managed' : operationCapabilities ? 'unavailable' : 'checking'}</strong></div><div><span>EU migration</span><strong>{operationCapabilities?.hostedMigration?.available ? 'backup to hosted' : operationCapabilities ? 'provider recovery' : 'checking'}</strong></div><div><span>Recovery</span><strong>{recoveryLabel}</strong></div><div><span>Data residency</span><strong>{data.health.runtime?.hosting?.dataResidency || 'local'}</strong></div><div><span>DPA</span><strong>{data.health.runtime?.mode === 'hosted' ? data.health.runtime?.hosting?.dpaConfigured ? 'configured' : 'attention' : 'not applicable'}</strong></div><div><span>Ledger</span><strong>{data.health.services.ledger}</strong></div><div><span>Diagnostics</span><strong>{data.health.diagnostics.issueCount} issue(s)</strong></div></div>
            <p className="panel-copy">Hosted mode fails closed until EU object storage, managed PostgreSQL, HTTPS, authentication, DPA, residency, and recovery declarations are ready.</p>
          </section>
          <section className="panel page-panel">
            <div className="panel-heading"><div><h2>Data safety</h2><p>{localBackupAvailable ? 'Create a verified recovery package before operational maintenance.' : 'Hosted recovery is controlled by the configured provider policy.'}</p></div></div>
            <div className="operations-actions">
              <a className="secondary-button" href="/api/operations/export"><FileDown size={16} />Export ledger</a>
              <button className="secondary-button" disabled={submitting || !localBackupAvailable} title={localBackupAvailable ? 'Create verified local backup' : 'Hosted backups are provider-managed'} onClick={backup}><FolderArchive size={16} />Create backup</button>
              <input ref={exportInputRef} className="visually-hidden" type="file" accept="application/json" onChange={event => validateExport(event.target.files?.[0])} />
              <button className="secondary-button" disabled={submitting || !exportValidationAvailable} title="Verify the checksum of a human-readable ledger export" onClick={() => exportInputRef.current?.click()}><FileUp size={16} />Validate export</button>
              <button className="danger-button" disabled={submitting || !localBackupAvailable} title={localBackupAvailable ? 'Archive QA records after backup' : 'Hosted maintenance requires a provider recovery point'} onClick={resetQa}><Archive size={16} />Archive QA records</button>
            </div>
            <p className="panel-copy">The JSON export is a checksummed, human-readable reconciliation record. It cannot restore the database or evidence files.</p>
            {data.backups?.length ? <div className="backup-list">{data.backups.slice(0, 5).map(backup => <div className="compact-row" key={backup.backupId}><FolderArchive size={16} /><div><strong>{formatDate(backup.createdAt)}</strong><small>{backup.databaseMode} / {backup.files} protected file(s) / {backup.evidenceFiles || 0} evidence</small></div><span className="backup-actions"><button className="secondary-button" title="Verify file checksums and SQLite restore readiness" disabled={submitting || backup.manifestStatus === 'unreadable'} onClick={() => verifyBackup(backup.backupId)}><ShieldCheck size={15} />Check restore</button>{backup.downloadAvailable ? <a className="secondary-button" href={`/api/operations/backups/${encodeURIComponent(backup.backupId)}/download`} download><FileDown size={15} />Download</a> : null}</span></div>)}</div> : <p className="panel-copy">{localBackupAvailable ? 'No controlled local backup has been created yet.' : `Provider recovery: ${recoveryLabel}.`}</p>}
            <p className="panel-copy">{localBackupAvailable ? 'Local backup v2 is the recovery artifact: it retains the SQLite ledger and private evidence with checksums. Download it to encrypted off-device storage; restore it only while the application is stopped.' : 'Application-local packages are disabled in hosted mode because they cannot contain managed PostgreSQL and private object-store data.'}</p>
          </section>
          ...
          <section className="panel page-panel archive-registry-panel" data-testid="job-archive-registry">
            <div className="panel-heading"><div><h2>Archived jobs</h2><p>Retained work removed from active operations, with approval-gated restore.</p></div><span className="count-badge">{archivedJobs.length}</span></div>
            {archivedJobs.length ? <div className="archive-registry-list">{archivedJobs.map(job => {
              const pendingRestore = approvals.find(approval => approval.jobId === job.id && approval.targetType === 'job_restore')
              return <div className="archive-registry-row" key={job.id}>
                <span className="archive-registry-icon"><Archive size={16} /></span>
                <div className="archive-registry-copy"><strong>{job.title}</strong><small>{job.clientName || 'Client pending'} · archived {formatDate(job.data?.archive?.approvedAt || job.updatedAt)} · from {formatStatus(job.data?.archive?.previousStatus || 'retained state')}</small>{job.data?.archive?.reason ? <p>{job.data.archive.reason}</p> : null}</div>
                <button className="secondary-button" disabled={submitting || Boolean(pendingRestore)} onClick={() => openJobLifecycle('restore', job)}><RefreshCw size={15} />{pendingRestore ? 'Restore pending' : 'Request restore'}</button>
              </div>
            })}</div> : <Empty title="No archived jobs" detail="Approved archives will remain available here for controlled restore." />}
          </section>
        </section>}
      </> : null}
    </main>

    {jobLifecycleAction ? <div className="modal-backdrop job-lifecycle-backdrop" role="presentation">
      <section className="modal job-lifecycle-modal" role="dialog" aria-modal="true" aria-labelledby="job-lifecycle-title" data-testid="job-lifecycle-modal">
        <div className="modal-heading">
          <div><p className="eyebrow">Approval-gated lifecycle</p><h2 id="job-lifecycle-title">{jobLifecycleAction.mode === 'archive' ? 'Request job archive' : 'Request job restore'}</h2><p>{jobLifecycleAction.job.title} · {jobLifecycleAction.job.clientName || 'Client record'}</p></div>
          <button className="icon-button" aria-label="Close job lifecycle request" onClick={closeJobLifecycle}><X size={18} /></button>
        </div>
        <form onSubmit={submitJobLifecycle}>
          <div className="job-lifecycle-body">
            <div className="job-lifecycle-effect"><Archive size={20} /><div><strong>{jobLifecycleAction.mode === 'archive' ? 'Remove from active operations' : `Return to ${formatStatus(jobLifecycleAction.job.data?.archive?.previousStatus || 'retained state')}`}</strong><p>{jobLifecycleAction.mode === 'archive' ? 'Schedules, queues, conflicts, automation, and rollups stop including this job only after approval.' : 'Applicable internal queues include the job again only after approval and current operating checks still apply.'}</p></div></div>
            <ul className="job-lifecycle-safeguards">
              ...
              {jobLifecycleAction.mode === 'archive' ? <li>Approval makes the job read-only and revokes active client portal links. Restore never reactivates those links.</li> : null}
              <li>{jobLifecycleAction.mode === 'archive' ? 'Returning the job later requires a separate restore approval.' : 'The archive and restore decisions remain visible in the audit history.'}</li>
            </ul>
            <label>Operational reason<textarea required minLength="8" value={jobLifecycleReason} onChange={event => setJobLifecycleReason(event.target.value)} placeholder={jobLifecycleAction.mode === 'archive' ? 'Explain why this job should leave active operations.' : 'Explain why this retained job should return to operations.'} /></label>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeJobLifecycle}>Cancel</button><button className={jobLifecycleAction.mode === 'archive' ? 'danger-button' : 'primary-button'} disabled={submitting || jobLifecycleReason.trim().length < 8}>{jobLifecycleAction.mode === 'archive' ? <Archive size={16} /> : <RefreshCw size={16} />}{submitting ? 'Submitting...' : jobLifecycleAction.mode === 'archive' ? 'Request archive approval' : 'Request restore approval'}</button></div>
        </form>
      </section>
    </div> : null}

    {workerEditor ? <div className="modal-backdrop worker-backdrop" role="presentation">
      <section className="modal worker-modal" role="dialog" aria-modal="true" aria-labelledby="worker-editor-title" data-testid="worker-editor" onKeyDown={event => { if (event.key === 'Escape') closeWorkerEditor() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Retained crew record</p><h2 id="worker-editor-title">{workerDraft.id ? `Edit ${workerDraft.name || 'crew member'}` : 'Add crew member'}</h2><p>Identity, availability, skills, and internal cost evidence for resource planning.</p></div>
          <button className="icon-button" aria-label="Close crew member editor" onClick={closeWorkerEditor}><X size={18} /></button>
        </div>
        <form onSubmit={saveWorker}>
          <div className="form-grid worker-form">
            <label className="form-span">Full name<input autoFocus required minLength="2" value={workerDraft.name} onChange={event => setWorkerDraft({ ...workerDraft, name: event.target.value })} /></label>
            <label>Role or trade<input value={workerDraft.role} onChange={event => setWorkerDraft({ ...workerDraft, role: event.target.value })} placeholder="Carpenter, site lead, electrician" /></label>
            <label>Availability status<select value={workerDraft.status} onChange={event => setWorkerDraft({ ...workerDraft, status: event.target.value })}><option value="available">Available</option><option value="busy">Busy</option><option value="traveling">Traveling</option><option value="offline">Offline</option><option value="on_leave">On leave</option><option value="on_hold">On hold</option><option value="inactive">Inactive</option></select></label>
            <label>Email<input type="email" value={workerDraft.email} onChange={event => setWorkerDraft({ ...workerDraft, email: event.target.value })} /></label>
            <label>Phone<input value={workerDraft.phone} onChange={event => setWorkerDraft({ ...workerDraft, phone: event.target.value })} /></label>
            <label>Home region<input value={workerDraft.homeRegion} onChange={event => setWorkerDraft({ ...workerDraft, homeRegion: event.target.value })} placeholder="Utrecht, Randstad, Noord-Brabant" /></label>
            <label>Hourly cost rate (EUR)<input type="number" min="0" step="0.01" value={workerDraft.hourlyRate} onChange={event => setWorkerDraft({ ...workerDraft, hourlyRate: event.target.value })} /></label>
            <label className="form-span">Skills<input value={workerDraft.skills} onChange={event => setWorkerDraft({ ...workerDraft, skills: event.target.value })} placeholder="Carpentry, renovation, VCA" /></label>
            <label className="form-span">Internal notes<textarea value={workerDraft.notes} onChange={event => setWorkerDraft({ ...workerDraft, notes: event.target.value })} placeholder="Record restrictions, qualifications, or resource planning context." /></label>
            <p className="workflow-note form-span">Saving this record does not contact the crew member, submit payroll, clear site access, or commit a schedule.</p>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeWorkerEditor}>Cancel</button><button className="primary-button" disabled={submitting || workerDraft.name.trim().length < 2 || (workerDraft.hourlyRate !== '' && Number(workerDraft.hourlyRate) < 0)}><ShieldCheck size={16} />{submitting ? 'Saving...' : 'Save retained crew member'}</button></div>
        </form>
      </section>
    </div> : null}

    {workerRetirement ? <div className="modal-backdrop worker-backdrop" role="presentation">
      <section className="modal worker-retirement-modal" role="dialog" aria-modal="true" aria-labelledby="worker-retirement-title" data-testid="worker-retirement-modal" onKeyDown={event => { if (event.key === 'Escape') closeWorkerRetirement() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Approval-gated lifecycle</p><h2 id="worker-retirement-title">Request crew retirement</h2><p>{workerRetirement.name} / {workerRetirement.role || 'Role not retained'}</p></div>
          <button className="icon-button" aria-label="Close crew retirement request" onClick={closeWorkerRetirement}><X size={18} /></button>
        </div>
        <form onSubmit={requestWorkerRetirement}>
          <div className="job-lifecycle-body">
            <div className="job-lifecycle-effect"><Archive size={20} /><div><strong>Block this crew member from new assignments</strong><p>The retained status changes to retired only after approval and after all operational assignments have been released or reassigned.</p></div></div>
            <ul className="job-lifecycle-safeguards">
              ...
              <li>{workerRetirement.activeAssignmentCount > 0 ? `Approval cannot complete while ${workerRetirement.activeAssignmentCount} operational assignment${workerRetirement.activeAssignmentCount === 1 ? '' : 's'} remain.` : 'No operational assignments currently block this retirement.'}</li>
              {workerRetirement.dormantAssignmentCount > 0 ? <li>Approval will release {workerRetirement.dormantAssignmentCount} dormant assignment{workerRetirement.dormantAssignmentCount === 1 ? '' : 's'} retained on inactive jobs, so any later job restore requires reassignment.</li> : null}
              <li>No crew member, client, payroll provider, or site contact is notified.</li>
            </ul>
            <label>Operational reason<textarea autoFocus required minLength="8" value={workerRetirementReason} onChange={event => setWorkerRetirementReason(event.target.value)} placeholder="Explain why this person should no longer be selected for new work." /></label>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeWorkerRetirement}>Cancel</button><button className="danger-button" disabled={submitting || workerRetirementReason.trim().length < 8}><Archive size={16} />{submitting ? 'Submitting...' : 'Request retirement approval'}</button></div>
        </form>
      </section>
    </div> : null}

    {equipmentEditor ? <div className="modal-backdrop equipment-backdrop" role="presentation">
      <section className="modal equipment-modal" role="dialog" aria-modal="true" aria-labelledby="equipment-editor-title" data-testid="equipment-editor" onKeyDown={event => { if (event.key === 'Escape') closeEquipmentEditor() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Retained equipment record</p><h2 id="equipment-editor-title">{equipmentDraft.id ? `Edit ${equipmentDraft.name || 'equipment'}` : 'Add equipment'}</h2><p>Identity, condition, location, inspection, and internal reference evidence for resource planning.</p></div>
          <button type="button" className="icon-button" aria-label="Close equipment editor" onClick={closeEquipmentEditor}><X size={18} /></button>
        </div>
        <form onSubmit={saveEquipment}>
          <div className="form-grid equipment-form">
            <label className="form-span">Equipment name<input autoFocus required minLength="2" value={equipmentDraft.name} onChange={event => setEquipmentDraft({ ...equipmentDraft, name: event.target.value })} placeholder="Site laser, tower scaffold, service van" /></label>
            <label>Category<input required value={equipmentDraft.category} onChange={event => setEquipmentDraft({ ...equipmentDraft, category: event.target.value })} placeholder="Measurement, access, vehicle" /></label>
            <label>Operational status<select value={equipmentDraft.status} onChange={event => setEquipmentDraft({ ...equipmentDraft, status: event.target.value })}><option value="available">Available</option><option value="in_use">In use</option><option value="maintenance">Maintenance</option><option value="inspection_due">Inspection due</option><option value="inactive">Inactive</option><option value="lost">Lost</option></select></label>
            <label>Home location<input value={equipmentDraft.homeLocation} onChange={event => setEquipmentDraft({ ...equipmentDraft, homeLocation: event.target.value })} placeholder="Utrecht depot" /></label>
            <label>Current location<input value={equipmentDraft.currentLocation} onChange={event => setEquipmentDraft({ ...equipmentDraft, currentLocation: event.target.value })} placeholder="Depot, site, or vehicle" /></label>
            <label>Serial or asset reference<input value={equipmentDraft.serialNumber} onChange={event => setEquipmentDraft({ ...equipmentDraft, serialNumber: event.target.value })} /></label>
            <label className="checkbox-label"><input type="checkbox" checked={equipmentDraft.inspectionRequired} onChange={event => setEquipmentDraft({ ...equipmentDraft, inspectionRequired: event.target.checked, inspectionDueAt: event.target.checked ? equipmentDraft.inspectionDueAt : '' })} />Inspection required before reservation</label>
            <label>Inspection due<input type="date" required={equipmentDraft.inspectionRequired} disabled={!equipmentDraft.inspectionRequired} value={equipmentDraft.inspectionDueAt} onChange={event => setEquipmentDraft({ ...equipmentDraft, inspectionDueAt: event.target.value })} /></label>
            <label className="form-span">Internal notes<textarea value={equipmentDraft.notes} onChange={event => setEquipmentDraft({ ...equipmentDraft, notes: event.target.value })} placeholder="Record condition, restrictions, certificates, or planning context." /></label>
            <p className="workflow-note form-span">Saving this record does not reserve, dispatch, inspect, purchase, or assign equipment.</p>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeEquipmentEditor}>Cancel</button><button className="primary-button" disabled={submitting || equipmentDraft.name.trim().length < 2 || !equipmentDraft.category.trim() || (equipmentDraft.inspectionRequired && !equipmentDraft.inspectionDueAt)}><ShieldCheck size={16} />{submitting ? 'Saving...' : 'Save retained equipment'}</button></div>
        </form>
      </section>
    </div> : null}

    {equipmentInspection ? <div className="modal-backdrop equipment-backdrop" role="presentation">
      <section className="modal equipment-inspection-modal" role="dialog" aria-modal="true" aria-labelledby="equipment-inspection-title" data-testid="equipment-inspection-modal" onKeyDown={event => { if (event.key === 'Escape') closeEquipmentInspection() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Internal inspection evidence</p><h2 id="equipment-inspection-title">Record equipment inspection</h2><p>{equipmentInspection.name} / {formatStatus(equipmentInspection.category || 'general')}</p></div>
          <button type="button" className="icon-button" aria-label="Close equipment inspection" onClick={closeEquipmentInspection}><X size={18} /></button>
        </div>
        <form onSubmit={recordEquipmentInspection}>
          <div className="form-grid equipment-inspection-form">
            <label>Inspection result<select autoFocus value={equipmentInspectionDraft.result} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, result: event.target.value })}><option value="passed">Passed</option><option value="limited">Limited</option><option value="failed">Failed</option></select></label>
            <label>Inspection date<input type="date" required max={new Date().toISOString().slice(0, 10)} value={equipmentInspectionDraft.inspectedAt} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, inspectedAt: event.target.value })} /></label>
            <label>Inspector or internal reference<input required minLength="2" value={equipmentInspectionDraft.inspector} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, inspector: event.target.value })} placeholder="Name or retained staff reference" /></label>
            <label>Next inspection due<input type="date" required={equipmentInspection.inspection?.required && equipmentInspectionDraft.result === 'passed'} disabled={equipmentInspectionDraft.result !== 'passed'} min={equipmentInspectionDraft.inspectedAt} value={equipmentInspectionDraft.nextDueAt} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, nextDueAt: event.target.value })} /></label>
            <label className="form-span">Evidence reference<input value={equipmentInspectionDraft.reference} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, reference: event.target.value })} placeholder="Checklist, document, or service reference" /></label>
            <label className="form-span">Findings<textarea required={equipmentInspectionDraft.result !== 'passed'} minLength={equipmentInspectionDraft.result !== 'passed' ? 8 : undefined} value={equipmentInspectionDraft.notes} onChange={event => setEquipmentInspectionDraft({ ...equipmentInspectionDraft, notes: event.target.value })} placeholder="Retain findings, restrictions, or follow-up work." /></label>
            <p className="workflow-note form-span">This creates internal operational evidence only. It does not claim statutory inspection, certification, supplier service, or external approval. Failed and limited results block new reservations.{['failed', 'limited'].includes(equipmentInspection.inspection?.status) ? ' Retain completed maintenance before recording a passing reinspection.' : equipmentInspection.inspection?.status === 'reinspection_required' ? ' Completed maintenance is retained; record a passing reinspection before reservation readiness can return.' : ''}</p>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeEquipmentInspection}>Cancel</button><button className="primary-button" disabled={submitting || equipmentInspectionDraft.inspector.trim().length < 2 || !equipmentInspectionDraft.inspectedAt || (equipmentInspection.inspection?.required && equipmentInspectionDraft.result === 'passed' && !equipmentInspectionDraft.nextDueAt) || (equipmentInspectionDraft.result !== 'passed' && equipmentInspectionDraft.notes.trim().length < 8)}><ClipboardCheck size={16} />{submitting ? 'Recording...' : 'Record inspection'}</button></div>
        </form>
      </section>
    </div> : null}

    {equipmentMaintenance ? <div className="modal-backdrop equipment-backdrop" role="presentation">
      <section className="modal equipment-maintenance-modal" role="dialog" aria-modal="true" aria-labelledby="equipment-maintenance-title" data-testid="equipment-maintenance-modal" onKeyDown={event => { if (event.key === 'Escape') closeEquipmentMaintenance() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Internal maintenance evidence</p><h2 id="equipment-maintenance-title">Record equipment maintenance</h2><p>{equipmentMaintenance.name} / {formatStatus(equipmentMaintenance.category || 'general')}</p></div>
          <button type="button" className="icon-button" aria-label="Close equipment maintenance" onClick={closeEquipmentMaintenance}><X size={18} /></button>
        </div>
        <form onSubmit={recordEquipmentMaintenance}>
          <div className="form-grid equipment-maintenance-form">
            <label>Maintenance outcome<select autoFocus value={equipmentMaintenanceDraft.outcome} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, outcome: event.target.value })}><option value="completed">Completed</option><option value="follow_up_required">Follow up required</option></select></label>
            <label>Maintenance type<select value={equipmentMaintenanceDraft.maintenanceType} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, maintenanceType: event.target.value })}><option value="corrective">Corrective</option><option value="preventive">Preventive</option><option value="repair">Repair</option><option value="service">Service</option></select></label>
            <label>Maintenance date<input type="date" required max={new Date().toISOString().slice(0, 10)} value={equipmentMaintenanceDraft.performedAt} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, performedAt: event.target.value })} /></label>
            <label>Person or internal reference<input required minLength="2" value={equipmentMaintenanceDraft.performedBy} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, performedBy: event.target.value })} placeholder="Name or retained staff reference" /></label>
            <label className="form-span">Evidence reference<input value={equipmentMaintenanceDraft.reference} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, reference: event.target.value })} placeholder="Work order, checklist, or internal reference" /></label>
            <label className="form-span">Work performed<textarea required minLength="8" value={equipmentMaintenanceDraft.notes} onChange={event => setEquipmentMaintenanceDraft({ ...equipmentMaintenanceDraft, notes: event.target.value })} placeholder="Retain the work completed, remaining defect, or follow-up requirement." /></label>
            <p className="workflow-note form-span">This retains internal maintenance evidence only. It does not place a supplier order, commit spend, claim external service, or clear a failed inspection. A passing reinspection is required after failed or limited inspection evidence.</p>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeEquipmentMaintenance}>Cancel</button><button className="primary-button" disabled={submitting || equipmentMaintenanceDraft.performedBy.trim().length < 2 || !equipmentMaintenanceDraft.performedAt || equipmentMaintenanceDraft.notes.trim().length < 8}><Wrench size={16} />{submitting ? 'Recording...' : 'Record maintenance'}</button></div>
        </form>
      </section>
    </div> : null}

    {equipmentRetirement ? <div className="modal-backdrop equipment-backdrop" role="presentation">
      <section className="modal equipment-retirement-modal" role="dialog" aria-modal="true" aria-labelledby="equipment-retirement-title" data-testid="equipment-retirement-modal" onKeyDown={event => { if (event.key === 'Escape') closeEquipmentRetirement() }}>
        <div className="modal-heading">
          <div><p className="eyebrow">Approval-gated lifecycle</p><h2 id="equipment-retirement-title">Request equipment retirement</h2><p>{equipmentRetirement.name} / {formatStatus(equipmentRetirement.category || 'general')}</p></div>
          <button type="button" className="icon-button" aria-label="Close equipment retirement request" onClick={closeEquipmentRetirement}><X size={18} /></button>
        </div>
        <form onSubmit={requestEquipmentRetirement}>
          <div className="job-lifecycle-body">
            <div className="job-lifecycle-effect"><Archive size={20} /><div><strong>Block this equipment from new reservations</strong><p>The retained status changes to retired only after approval and after every operational reservation has been released.</p></div></div>
            <ul className="job-lifecycle-safeguards">
              ...
              <li>{equipmentRetirement.activeReservationCount > 0 ? `Approval cannot complete while ${equipmentRetirement.activeReservationCount} operational reservation${equipmentRetirement.activeReservationCount === 1 ? '' : 's'} remain.` : 'No operational reservations currently block this retirement.'}</li>
              {equipmentRetirement.dormantReservationCount > 0 ? <li>Approval will release {equipmentRetirement.dormantReservationCount} dormant reservation{equipmentRetirement.dormantReservationCount === 1 ? '' : 's'} retained on inactive jobs, so any later job restore requires a new reservation.</li> : null}
              <li>No crew member, client, supplier, finance provider, or site contact is notified.</li>
            </ul>
            <label>Operational reason<textarea autoFocus required minLength="8" value={equipmentRetirementReason} onChange={event => setEquipmentRetirementReason(event.target.value)} placeholder="Explain why this equipment should no longer be selected for work." /></label>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeEquipmentRetirement}>Cancel</button><button className="danger-button" disabled={submitting || equipmentRetirementReason.trim().length < 8}><Archive size={16} />{submitting ? 'Submitting...' : 'Request retirement approval'}</button></div>
        </form>
      </section>
    </div> : null}

    {tradePartnerEditor ? <div className="modal-backdrop trade-partner-backdrop" role="presentation">
      <section className="modal trade-partner-modal" role="dialog" aria-modal="true" aria-labelledby="trade-partner-editor-title" data-testid="trade-partner-editor">
        <div className="modal-heading">
          <div><p className="eyebrow">Retained commercial evidence</p><h2 id="trade-partner-editor-title">{tradePartnerDraft.id ? `Edit ${tradePartnerDraft.name || 'trade partner'}` : 'Add trade partner'}</h2><p>Identity and compliance evidence used by approval-gated purchasing.</p></div>
          <button className="icon-button" aria-label="Close trade partner editor" onClick={closeTradePartnerEditor}><X size={18} /></button>
        </div>
        <form onSubmit={saveTradePartner}>
          <div className="form-grid trade-partner-form">
            <label className="form-span">Legal or trading name<input required value={tradePartnerDraft.name} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, name: event.target.value })} /></label>
            <label>Partner type<select value={tradePartnerDraft.partnerType} onChange={event => {
              const partnerType = event.target.value
              setTradePartnerDraft({ ...tradePartnerDraft, partnerType, requiresInsurance: tradePartnerDraft.requiresInsurance || ['subcontractor', 'both'].includes(partnerType) })
            }}><option value="supplier">Supplier</option><option value="subcontractor">Subcontractor</option><option value="both">Supplier and subcontractor</option></select></label>
            <label>Internal status<select value={tradePartnerDraft.status} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, status: event.target.value })}><option value="active">Active</option><option value="on_hold">On hold</option></select></label>
            <label>Contact name<input value={tradePartnerDraft.contactName} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, contactName: event.target.value })} /></label>
            <label>Email<input type="email" value={tradePartnerDraft.email} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, email: event.target.value })} /></label>
            <label>Phone<input value={tradePartnerDraft.phone} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, phone: event.target.value })} /></label>
            <label>City<input value={tradePartnerDraft.city} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, city: event.target.value })} /></label>
            <label>Country<input required maxLength="2" value={tradePartnerDraft.country} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, country: event.target.value.toUpperCase() })} placeholder="NL" /></label>
            <label>Registration / KVK<input value={tradePartnerDraft.registrationNumber} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, registrationNumber: event.target.value })} /></label>
            <label>VAT number<input disabled={tradePartnerDraft.vatExempt} value={tradePartnerDraft.vatNumber} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, vatNumber: event.target.value })} /></label>
            <label className="form-span checkbox-label"><input type="checkbox" checked={tradePartnerDraft.vatExempt} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, vatExempt: event.target.checked })} />VAT exemption has been verified</label>
            <label className="form-span">Specialties<input value={tradePartnerDraft.specialties} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, specialties: event.target.value })} placeholder="Roofing, electrical, timber" /></label>
            <label className="checkbox-label"><input type="checkbox" checked={tradePartnerDraft.requiresInsurance} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, requiresInsurance: event.target.checked })} />Insurance evidence required</label>
            <label>Insurance expiry<input type="date" disabled={!tradePartnerDraft.requiresInsurance} required={tradePartnerDraft.requiresInsurance} value={tradePartnerDraft.insuranceExpiresAt} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, insuranceExpiresAt: event.target.value })} /></label>
            <label className="checkbox-label"><input type="checkbox" checked={tradePartnerDraft.requiresVca} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, requiresVca: event.target.checked })} />VCA evidence required</label>
            <label>VCA expiry<input type="date" disabled={!tradePartnerDraft.requiresVca} required={tradePartnerDraft.requiresVca} value={tradePartnerDraft.vcaExpiresAt} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, vcaExpiresAt: event.target.value })} /></label>
            <label>Verification reference<input value={tradePartnerDraft.verificationReference} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, verificationReference: event.target.value })} placeholder="Registry check or evidence record" /></label>
            <label>Verified on<input type="date" value={tradePartnerDraft.verifiedAt} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, verifiedAt: event.target.value })} /></label>
            <label className="form-span">Internal notes<textarea value={tradePartnerDraft.notes} onChange={event => setTradePartnerDraft({ ...tradePartnerDraft, notes: event.target.value })} placeholder="Record scope, restrictions, or evidence context." /></label>
            <p className="workflow-note form-span">Saving this record does not contact the partner, place an order, commit spend, or approve site access.</p>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeTradePartnerEditor}>Cancel</button><button className="primary-button" disabled={submitting || !tradePartnerDraft.name.trim()}><ShieldCheck size={16} />{submitting ? 'Saving...' : 'Save retained partner'}</button></div>
        </form>
      </section>
    </div> : null}

    {tradePartnerRetirement ? <div className="modal-backdrop trade-partner-backdrop" role="presentation">
      <section className="modal trade-partner-retirement-modal" role="dialog" aria-modal="true" aria-labelledby="trade-partner-retirement-title" data-testid="trade-partner-retirement-modal">
        <div className="modal-heading">
          <div><p className="eyebrow">Approval-gated lifecycle</p><h2 id="trade-partner-retirement-title">Request partner retirement</h2><p>{tradePartnerRetirement.name} / {formatStatus(tradePartnerRetirement.partnerType)}</p></div>
          <button className="icon-button" aria-label="Close trade partner retirement request" onClick={closeTradePartnerRetirement}><X size={18} /></button>
        </div>
        <form onSubmit={requestTradePartnerRetirement}>
          <div className="job-lifecycle-body">
            <div className="job-lifecycle-effect"><Archive size={20} /><div><strong>Block this partner from new procurement</strong><p>The status changes to retired only after approval. Open work remains visible for operator review.</p></div></div>
            <ul className="job-lifecycle-safeguards">
            ...
            <label>Operational reason<textarea required minLength="8" value={tradePartnerRetirementReason} onChange={event => setTradePartnerRetirementReason(event.target.value)} placeholder="Explain why this partner should no longer be selected for new work." /></label>
          </div>
          <div className="modal-actions"><button type="button" className="secondary-button" onClick={closeTradePartnerRetirement}>Cancel</button><button className="danger-button" disabled={submitting || tradePartnerRetirementReason.trim().length < 8}><Archive size={16} />{submitting ? 'Submitting...' : 'Request retirement approval'}</button></div>
        </form>
      </section>
    </div> : null}

    {approvalReview ? <div className="modal-backdrop" role="presentation">
      <section className="modal approval-review-modal" role="dialog" aria-modal="true" aria-labelledby="approval-review-title" data-testid="approval-review-modal">
        <div className="modal-heading">
          <div><p className="eyebrow">Retained decision review</p><h2 id="approval-review-title">{approvalReview.status === 'approved' ? 'Approve decision' : 'Reject decision'}</h2><p>{approvalReview.item.summary || formatStatus(approvalReview.item.targetType)}</p></div>
          <button className="icon-button" aria-label="Close approval review" onClick={closeApprovalReview}><X size={18} /></button>
        </div>
        <form onSubmit={resolveApproval}>
          <div className="approval-review-body">
            <div className="approval-review-primary"><TriangleAlert size={20} /><div><span className={`tag tag-${approvalReview.item.decision?.riskLevel === 'high' ? 'amber' : 'green'}`}>{approvalReview.item.decision?.riskLevel || 'review'} risk</span><strong>{approvalReview.item.decision?.primaryEffect || approvalReview.item.reason || 'Resolve this retained ledger decision.'}</strong></div></div>
    ...
    {resourceAction ? <div className="modal-backdrop" role="presentation">
      <section className="modal resource-control-modal" role="dialog" aria-modal="true" aria-labelledby="resource-control-title" data-testid="resource-control-modal">
        <div className="modal-heading">
          <div>
            <p className="eyebrow">Retained resource control</p>
            <h2 id="resource-control-title">{resourceAction.label}</h2>
            <p>{resourceAction.item.jobTitle} / {resourceAction.material?.name || resourceAction.procurementOrder?.supplier || resourceActionDraft.workerName || 'Resource record'}</p>
          </div>
          <button className="icon-button" aria-label="Close resource control" onClick={closeResourceControl}><X size={18} /></button>
        </div>
        <form onSubmit={submitResourceControl}>
          <div className="form-grid">
            ...
            {['complete_worker_orientation', 'prepare_site_access'].includes(resourceAction.action.type) ? <>
              <label>Worker name<input required value={resourceActionDraft.workerName} onChange={event => setResourceActionDraft({ ...resourceActionDraft, workerName: event.target.value })} /></label>
              <label>Company<input required value={resourceActionDraft.company} onChange={event => setResourceActionDraft({ ...resourceActionDraft, company: event.target.value })} /></label>
              {resourceAction.action.type === 'complete_worker_orientation' ? <label className="form-span">Orientation verification reference<input required value={resourceActionDraft.reference} onChange={event => setResourceActionDraft({ ...resourceActionDraft, reference: event.target.value })} placeholder="Induction record, credential, or signed evidence reference" /></label> : null}
            </> : null}

            {resourceAction.action.type === 'record_time_log' ? <>
              <label>Worker name<input required value={resourceActionDraft.workerName} onChange={event => setResourceActionDraft({ ...resourceActionDraft, workerName: event.target.value })} /></label>
              <label>Work date<input required type="date" value={resourceActionDraft.workDate} onChange={event => setResourceActionDraft({ ...resourceActionDraft, workDate: event.target.value })} /></label>
              <label>Hours<input required type="number" min="0.25" max="24" step="0.25" value={resourceActionDraft.hours} onChange={event => setResourceActionDraft({ ...resourceActionDraft, hours: event.target.value })} /></label>
              <label>Hourly rate (EUR)<input required type="number" min="0" step="0.01" value={resourceActionDraft.rate} onChange={event => setResourceActionDraft({ ...resourceActionDraft, rate: event.target.value })} /></label>
              <label>Cost code<input value={resourceActionDraft.costCode} onChange={event => setResourceActionDraft({ ...resourceActionDraft, costCode: event.target.value })} /></label>
              <label>Time evidence reference<input value={resourceActionDraft.reference} onChange={event => setResourceActionDraft({ ...resourceActionDraft, reference: event.target.value })} placeholder="Timesheet, field report, or supervisor reference" /></label>
            </> : null}

            {resourceAction.action.type === 'review_material_status' ? <>
              <label>Internal state<select value={resourceActionDraft.materialStatus} onChange={event => setResourceActionDraft({ ...resourceActionDraft, materialStatus: event.target.value })}><option value="available">Available</option><option value="received">Received</option><option value="allocated">Allocated to job</option><option value="loaded">Loaded for dispatch</option><option value="used">Used on job</option></select></label>
              <label>Verified quantity<input required type="number" min="0.01" step="0.01" value={resourceActionDraft.availableQuantity} onChange={event => setResourceActionDraft({ ...resourceActionDraft, availableQuantity: event.target.value })} /></label>
              <label>Storage or job location<input required value={resourceActionDraft.location} onChange={event => setResourceActionDraft({ ...resourceActionDraft, location: event.target.value })} placeholder="Warehouse bay, vehicle, or job area" /></label>
              <label>Verification reference<input required value={resourceActionDraft.reference} onChange={event => setResourceActionDraft({ ...resourceActionDraft, reference: event.target.value })} placeholder="Delivery note, count sheet, or inspection reference" /></label>
            </> : null}

            {resourceAction.action.type === 'request_procurement_approval' ? <>
              <label className="form-span">Trade partner<select required value={resourceActionDraft.tradePartnerId} onChange={event => {
                const partner = tradePartners.find(candidate => candidate.id === event.target.value)
                setResourceActionDraft({ ...resourceActionDraft, tradePartnerId: partner?.id || '', supplier: partner?.name || '' })
              }}><option value="">Select a retained supplier</option>{tradePartners.filter(partner => partner.status !== 'retired').map(partner => <option key={partner.id} value={partner.id}>{partner.name} / {formatStatus(partner.compliance?.status || 'needs_review')}</option>)}</select></label>
              <div className="partner-selection-note form-span"><p>{resourceActionDraft.tradePartnerId ? 'Compliance is checked again when an approver resolves this request.' : 'A retained partner with current registration, tax, and required safety evidence is mandatory.'}</p><button type="button" className="secondary-button" onClick={openTradePartnerDirectory}><Building2 size={15} />Manage trade partners</button></div>
              <label>Order value (EUR)<input required type="number" min="0.01" step="0.01" value={resourceActionDraft.amount} onChange={event => setResourceActionDraft({ ...resourceActionDraft, amount: event.target.value })} /></label>
              <label>Required by<input required type="date" value={resourceActionDraft.requiredBy} onChange={event => setResourceActionDraft({ ...resourceActionDraft, requiredBy: event.target.value })} /></label>
            </> : null}

            <label className="form-span">Internal evidence and notes<textarea required value={resourceActionDraft.notes} onChange={event => setResourceActionDraft({ ...resourceActionDraft, notes: event.target.value })} placeholder="Record what was checked, by whom, and what this status can safely be used for." /></label>
            <p className="workflow-note form-span">This screen changes retained internal evidence only. It cannot clear site access without approval, contact a worker or supplier, place an order, commit spend, or submit payroll.</p>
          </div>
          <div className="modal-actions">
            <button type="button" className="secondary-button" onClick={closeResourceControl}>Cancel</button>
            <button className="primary-button" disabled={submitting || !resourceActionDraft.notes.trim() || (resourceAction.action.type === 'request_procurement_approval' && (!resourceActionDraft.tradePartnerId || !(Number(resourceActionDraft.amount) > 0) || !resourceActionDraft.requiredBy))}><ShieldCheck size={16} />{submitting ? 'Recording...' : resourceAction.action.type === 'complete_worker_orientation' ? 'Request orientation approval' : resourceAction.action.type === 'prepare_site_access' ? 'Create access gate' : resourceAction.action.type === 'request_procurement_approval' ? 'Request procurement approval' : 'Record internal evidence'}</button>
          </div>
        </form>
      </section>
    </div> : null}
    {invoiceJob ? <div className="modal-backdrop" role="presentation">
      <section className="modal invoice-modal" role="dialog" aria-modal="true" aria-labelledby="invoice-draft-title" data-testid="invoice-draft-modal">
        <div className="modal-heading">
          <div><p className="eyebrow">Approval-gated finance</p><h2 id="invoice-draft-title">{invoiceJob.billingMilestone ? 'Draft milestone invoice' : 'Draft invoice'}</h2><p>{invoiceJob.jobTitle} / {invoiceJob.clientName || 'Client not set'}</p></div>
        ...
        <form onSubmit={createInvoiceDraft}>
          <div className="form-grid">
            {invoiceJob.billingMilestone ? <div className="workflow-result form-span" data-testid="invoice-milestone-source"><div><strong>Milestone {invoiceJob.billingMilestone.sequenceNumber}: {invoiceJob.billingMilestone.title}</strong><span>The approved net amount, VAT rate, and payment date are locked to this retained billing plan.</span></div><span className="tag tag-green">Approved source</span></div> : null}
            <label>Net amount (EUR)<input required readOnly={Boolean(invoiceDraft.billingMilestoneId)} type="number" min="0.01" step="0.01" value={invoiceDraft.amount} onChange={event => setInvoiceDraft({ ...invoiceDraft, amount: event.target.value })} /></label>
            <label>VAT rate (%)<input required readOnly={Boolean(invoiceDraft.billingMilestoneId)} type="number" min="0" max="100" step="0.01" value={invoiceDraft.taxRate} onChange={event => setInvoiceDraft({ ...invoiceDraft, taxRate: event.target.value })} /></label>
            <label>Due date<input required readOnly={Boolean(invoiceDraft.billingMilestoneId)} type="date" value={invoiceDraft.dueAt} onChange={event => setInvoiceDraft({ ...invoiceDraft, dueAt: event.target.value })} /></label>
    ...
    {clientAction ? <div className="modal-backdrop" role="presentation">
      <section className="modal client-action-modal" role="dialog" aria-modal="true" aria-labelledby="client-action-title" data-testid="client-lifecycle-modal">
        <div className="modal-heading">
          <div><p className="eyebrow">Retained client lifecycle</p><h2 id="client-action-title">{clientAction.label}</h2><p>{clientAction.item.jobTitle} · {clientAction.item.clientName || 'Client record'}</p></div>
          <button className="icon-button" aria-label="Close client lifecycle review" onClick={closeClientLifecycle}><X size={18} /></button>
        </div>
        <form onSubmit={submitClientLifecycle}>
          <div className="form-grid">
            {clientAction.type === 'selection' ? <>
              <label>Retained option<select required value={clientActionOption} onChange={event => setClientActionOption(event.target.value)}><option value="">Select the confirmed option</option>{(clientAction.record?.options || []).map(option => <option key={option} value={option}>{option}</option>)}</select></label>
              <label>Client confirmation reference<input required value={clientActionReference} onChange={event => setClientActionReference(event.target.value)} placeholder="Portal reply, signed form, email, or call record" /></label>
            </> : null}
            <label className="form-span">Evidence and outcome<textarea required value={clientActionNotes} onChange={event => setClientActionNotes(event.target.value)} placeholder="Record what was inspected, completed, agreed, or still needs review." /></label>
            <p className="workflow-note form-span">{clientAction.type === 'aftercare'
              ...
              : clientAction.type === 'selection'
                ...
                : 'The requested resolution remains pending until an approver reviews it. No acceptance or warranty commitment is communicated from this screen.'}</p>
          </div>
          <div className="modal-actions">
            <button type="button" className="secondary-button" onClick={closeClientLifecycle}>Cancel</button>
            <button className="primary-button" disabled={submitting || !clientActionNotes.trim() || (clientAction.type === 'selection' && (!clientActionOption || !clientActionReference.trim()))}><ShieldCheck size={16} />{submitting ? 'Recording...' : clientAction.type === 'aftercare' ? 'Complete internal follow-up' : clientAction.type === 'selection' ? 'Request selection approval' : 'Request resolution approval'}</button>
          </div>
        </form>
      </section>
    </div> : null}
    {financeAction ? <div className="modal-backdrop" role="presentation">
      <section className="modal finance-control-modal" role="dialog" aria-modal="true" aria-labelledby="finance-control-title" data-testid="finance-control-modal">
        <div className="modal-heading">
          <div>
            <p className="eyebrow">Retained finance control</p>
            <h2 id="finance-control-title">{financeAction.label}</h2>
            <p>{financeAction.item.jobTitle} / {financeAction.item.clientName || 'Client finance record'}</p>
          </div>
          <button className="icon-button" aria-label="Close finance control" onClick={closeFinanceControl}><X size={18} /></button>
        </div>
        <form onSubmit={submitFinanceControl}>
          <div className="form-grid">
            ...
            {financeAction.action.type === 'create_credit_note' ? <>
              <label>Net credit amount (EUR)<input required type="number" min="0.01" max={financeAction.action.availableNetAmount || undefined} step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>VAT rate (%)<input readOnly value={financeCreditTaxRate} /></label>
              <label className="form-span">Credit-line description<input maxLength={500} value={financeActionDraft.description} onChange={event => setFinanceActionDraft({ ...financeActionDraft, description: event.target.value })} placeholder={`Correction for invoice ${financeAction.action.invoiceReference || ''}`} /></label>
              <div className="invoice-preview form-span" aria-label="Credit note calculation"><span>Net credit <strong>{currency.format(financeControlAmount)}</strong></span><span>VAT correction <strong>{currency.format(financeCreditTax)}</strong></span><span>Total credit <strong>{currency.format(financeCreditTotal)}</strong></span></div>
            ...
            {financeAction.action.type === 'record_payment_follow_up' ? <>
              ...
              <label>Receivable amount (EUR)<input required type="number" min="0.01" step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>Next follow-up date<input required type="date" value={financeActionDraft.dueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, dueAt: event.target.value })} /></label>
              <label>Internal channel<select value={financeActionDraft.followUpChannel} onChange={event => setFinanceActionDraft({ ...financeActionDraft, followUpChannel: event.target.value })}><option value="internal">Internal review</option><option value="phone">Phone evidence</option><option value="email">Email evidence</option><option value="portal">Portal evidence</option></select></label>
              {financeActionDraft.outcome !== 'follow_up_recorded' ? <label className="form-span">Payment or authority reference<input required value={financeActionDraft.reference} onChange={event => setFinanceActionDraft({ ...financeActionDraft, reference: event.target.value })} placeholder="Bank reference, statement ID, or approved write-off authority" /></label> : null}
            </> : null}

            {financeAction.action.type === 'prepare_finance_handoff' ? <>
              <label>Bookkeeping target<select value={financeActionDraft.targetSystem} onChange={event => setFinanceActionDraft({ ...financeActionDraft, targetSystem: event.target.value })}><option value="FAB">FAB</option><option value="Exact Online">Exact Online</option><option value="Twinfield">Twinfield</option><option value="AFAS">AFAS</option><option value="Internal accounting">Internal accounting</option></select></label>
              <label>Package format<select value={financeActionDraft.exportFormat} onChange={event => setFinanceActionDraft({ ...financeActionDraft, exportFormat: event.target.value })}><option value="json">Structured JSON</option><option value="csv">CSV package</option><option value="ubl">UBL metadata</option></select></label>
            </> : null}

            {financeAction.action.type === 'record_time_expense' ? <>
              <label>Record date<input required type="date" value={financeActionDraft.dueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, dueAt: event.target.value })} /></label>
              <label>Cost code<input value={financeActionDraft.costCode} onChange={event => setFinanceActionDraft({ ...financeActionDraft, costCode: event.target.value })} /></label>
              <label>Hours<input type="number" min="0" max="24" step="0.25" value={financeActionDraft.hours} onChange={event => setFinanceActionDraft({ ...financeActionDraft, hours: event.target.value })} /></label>
              <label>Hourly rate (EUR)<input type="number" min="0" step="0.01" value={financeActionDraft.rate} onChange={event => setFinanceActionDraft({ ...financeActionDraft, rate: event.target.value })} /></label>
              <label>Expense amount (EUR)<input type="number" min="0" step="0.01" value={financeActionDraft.expenseAmount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, expenseAmount: event.target.value })} /></label>
              <label>Expense category<select value={financeActionDraft.category} onChange={event => setFinanceActionDraft({ ...financeActionDraft, category: event.target.value })}><option value="materials">Materials</option><option value="equipment">Equipment</option><option value="subcontractor">Subcontractor</option><option value="travel">Travel</option><option value="general">General</option></select></label>
              {financeControlExpense > 0 ? <><label>Vendor<input required value={financeActionDraft.vendor} onChange={event => setFinanceActionDraft({ ...financeActionDraft, vendor: event.target.value })} /></label><label>Receipt reference<input value={financeActionDraft.reference} onChange={event => setFinanceActionDraft({ ...financeActionDraft, reference: event.target.value })} /></label></> : null}
            </> : null}

            {financeAction.action.type === 'create_budget_line' ? <>
              <label>Budget amount (EUR)<input required type="number" min="0.01" step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>Forecast amount (EUR)<input required type="number" min="0" step="0.01" value={financeActionDraft.forecastAmount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, forecastAmount: event.target.value })} /></label>
              <label>Cost code<input required value={financeActionDraft.costCode} onChange={event => setFinanceActionDraft({ ...financeActionDraft, costCode: event.target.value })} /></label>
              <label>Category<select value={financeActionDraft.category} onChange={event => setFinanceActionDraft({ ...financeActionDraft, category: event.target.value })}><option value="general">General</option><option value="labor">Labor</option><option value="materials">Materials</option><option value="equipment">Equipment</option><option value="subcontractor">Subcontractor</option></select></label>
              <label className="form-span">Budget description<input required value={financeActionDraft.description} onChange={event => setFinanceActionDraft({ ...financeActionDraft, description: event.target.value })} /></label>
            </> : null}

            {financeAction.action.type === 'create_billing_milestone' ? <>
              <label>Net milestone (EUR)<input required type="number" min="0.01" step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>VAT rate (%)<input required type="number" min="0" max="100" step="0.01" value={financeActionDraft.taxRate} onChange={event => setFinanceActionDraft({ ...financeActionDraft, taxRate: event.target.value })} /></label>
              <label>Planned invoice date<input required type="date" value={financeActionDraft.plannedIssueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, plannedIssueAt: event.target.value })} /></label>
              <label>Payment due date<input required type="date" value={financeActionDraft.dueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, dueAt: event.target.value })} /></label>
              <label className="form-span">Milestone description<input required value={financeActionDraft.description} onChange={event => setFinanceActionDraft({ ...financeActionDraft, description: event.target.value })} /></label>
              <div className="invoice-preview form-span" aria-label="Billing milestone calculation"><span>Net <strong>{currency.format(financeControlAmount)}</strong></span><span>VAT <strong>{currency.format(roundMoney(financeControlAmount * (Number(financeActionDraft.taxRate) || 0) / 100))}</strong></span><span>Total <strong>{currency.format(roundMoney(financeControlAmount * (1 + (Number(financeActionDraft.taxRate) || 0) / 100)))}</strong></span></div>
            </> : null}

            {financeAction.action.type === 'create_draw_request' ? <>
              <label>Draw amount (EUR)<input required type="number" min="0.01" step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>Work complete (%)<input required type="number" min="0" max="100" step="1" value={financeActionDraft.percentComplete} onChange={event => setFinanceActionDraft({ ...financeActionDraft, percentComplete: event.target.value })} /></label>
              <label>Review due date<input required type="date" value={financeActionDraft.dueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, dueAt: event.target.value })} /></label>
            </> : null}

            {financeAction.action.type === 'request_lien_waiver' ? <>
              <label>Waiver amount (EUR)<input required type="number" min="0.01" step="0.01" value={financeActionDraft.amount} onChange={event => setFinanceActionDraft({ ...financeActionDraft, amount: event.target.value })} /></label>
              <label>Supplier or party<input required value={financeActionDraft.vendor} onChange={event => setFinanceActionDraft({ ...financeActionDraft, vendor: event.target.value })} /></label>
              <label>Waiver type<select value={financeActionDraft.waiverType} onChange={event => setFinanceActionDraft({ ...financeActionDraft, waiverType: event.target.value })}><option value="conditional">Conditional</option><option value="unconditional">Unconditional</option></select></label>
              <label>Request due date<input required type="date" value={financeActionDraft.dueAt} onChange={event => setFinanceActionDraft({ ...financeActionDraft, dueAt: event.target.value })} /></label>
            </> : null}

            <label className="form-span">Internal evidence and notes<textarea required value={financeActionDraft.notes} onChange={event => setFinanceActionDraft({ ...financeActionDraft, notes: event.target.value })} placeholder="Record the source evidence, calculation, collection history, receipt, or review basis." /></label>
            <p className="workflow-note form-span">This screen only changes the retained ledger. It cannot move funds, contact a client or supplier, submit a draw, release a waiver, or export bookkeeping data.</p>
          </div>
          <div className="modal-actions">
            <button type="button" className="secondary-button" onClick={closeFinanceControl}>Cancel</button>
            <button className="primary-button" disabled={submitting || !financeActionDraft.notes.trim()}><ShieldCheck size={16} />{submitting ? 'Recording...' : financeAction.action.type === 'create_credit_note' ? 'Request credit-note approval' : financeAction.action.type === 'record_supplier_invoice' ? 'Request payable approval' : financeAction.action.type === 'record_supplier_payment' ? 'Request payment confirmation' : financeAction.action.type === 'create_billing_milestone' ? 'Request milestone approval' : financeAction.action.type === 'record_time_expense' ? 'Record ledger costs' : financeAction.action.type === 'request_lien_waiver' ? 'Retain waiver request' : financeAction.action.type === 'record_payment_follow_up' && financeActionDraft.outcome === 'follow_up_recorded' ? 'Record internal follow-up' : 'Request approver review'}</button>
          </div>
        </form>
      </section>
    </div> : null}
    {fieldAction ? <div className="modal-backdrop" role="presentation">
      <section className="modal field-action-modal" role="dialog" aria-modal="true" aria-labelledby="field-action-title" data-testid="field-assurance-modal">
        <div className="modal-heading">
          <div>
            <p className="eyebrow">Retained assurance review</p>
            <h2 id="field-action-title">{fieldAction.label}</h2>
            <p>{fieldAction.item.jobTitle} / {fieldAction.record?.title || fieldAction.record?.material || fieldAction.record?.workerName || formatStatus(fieldAction.type)}</p>
          </div>
          <button className="icon-button" aria-label="Close assurance review" onClick={closeFieldReview}><X size={18} /></button>
        </div>
        <form onSubmit={submitFieldReview}>
          <div className="form-grid">
            <div className="field-record-context form-span">
              ...
              {fieldAction.record?.workerName ? <p>Worker: {fieldAction.record.workerName}{fieldAction.record.company ? ' / ' + fieldAction.record.company : ''}</p> : null}
              {fieldAction.record?.hazards?.length ? <p>{fieldAction.record.hazards.length} hazard{fieldAction.record.hazards.length === 1 ? '' : 's'} retained with controls for review.</p> : null}
              {fieldAction.record?.defects?.length ? <p>{fieldAction.record.defects.length} retained defect{fieldAction.record.defects.length === 1 ? '' : 's'} require resolution evidence.</p> : null}
              {fieldAction.record?.expiresAt ? <p>Current expiry: {formatDate(fieldAction.record.expiresAt)}</p> : null}
            </div>
            {['permit', 'sds'].includes(fieldAction.type) ? <label>{fieldAction.type === 'sds' ? 'SDS expiry' : 'Proposed expiry'}<input required type="date" value={fieldActionDate} onChange={event => setFieldActionDate(event.target.value)} /></label> : null}
            {fieldAction.type === 'sds' ? <label className="form-span">SDS document reference<input required value={fieldActionReference} onChange={event => setFieldActionReference(event.target.value)} placeholder="Storage reference, controlled document ID, or verified source" /></label> : null}
            {fieldAction.type === 'safety_meeting' ? <label className="form-span">Attendees<input required value={fieldActionReference} onChange={event => setFieldActionReference(event.target.value)} placeholder="Names or retained attendance reference" /></label> : null}
            {fieldAction.type === 'orientation' ? <label className="form-span">Verification reference<input required value={fieldActionReference} onChange={event => setFieldActionReference(event.target.value)} placeholder="Induction record, credential, or checked evidence reference" /></label> : null}
            {fieldAction.type === 'document' ? <label className="form-span">Document review reference<input required value={fieldActionReference} onChange={event => setFieldActionReference(event.target.value)} placeholder="Controlled document revision, reviewer record, or storage reference" /></label> : null}
            <label className="form-span">{fieldAction.type === 'rfi' ? 'Response and evidence' : 'Evidence and decision'}
              <textarea required value={fieldActionNotes} onChange={event => setFieldActionNotes(event.target.value)} placeholder="Record the source, inspection, corrective work, or decision evidence an approver should rely on." />
            </label>
            <p className="workflow-note form-span">This records a proposed lifecycle transition. Field reliance, acceptance, publication, and external commitments remain blocked until the approval is resolved.</p>
          </div>
          <div className="modal-actions">
            <button type="button" className="secondary-button" onClick={closeFieldReview}>Cancel</button>
            <button className="primary-button" disabled={submitting || !fieldActionNotes.trim() || (['permit', 'sds'].includes(fieldAction.type) && !fieldActionDate) || (['sds', 'safety_meeting', 'orientation', 'document'].includes(fieldAction.type) && !fieldActionReference.trim())}><ShieldCheck size={16} />{submitting ? 'Recording...' : 'Request approver review'}</button>
          </div>
        </form>
      </section>
    </div> : null}
    {selectedJob && canCoordinate ? <button className="resource-launcher" type="button" aria-label="Open resource planner" onClick={() => setShowResourcePlanner(true)}><Users size={17} />Resources</button> : null}
    {showResourcePlanner && selectedJob ? <div className="modal-backdrop resource-backdrop" role="presentation"><section className="modal resource-planner" role="dialog" aria-modal="true" aria-labelledby="resource-planner-title" data-testid="resource-planner"><div className="modal-heading"><div><p className="eyebrow">Internal work plan</p><h2 id="resource-planner-title">Crew and equipment</h2><p>{selectedJob.title} · {formatDate(scheduleDraft.plannedStart)}</p></div><button className="icon-button" aria-label="Close resource planner" onClick={() => setShowResourcePlanner(false)}><X size={18} /></button></div><div className="resource-planner-body"><p className="workflow-note">Planning remains internal. Conflicts become approval records; this screen cannot contact crew, suppliers, or clients.</p><div className="resource-forms"><form className="form-grid compact-form" onSubmit={assignWorker}><label className="form-span">Crew member<select aria-label="Crew member" required value={resourceDraft.workerId} onChange={event => setResourceDraft({ ...resourceDraft, workerId: event.target.value })}><option value="">Select a ledger worker</option>{resourceOptions.workers.map(worker => <option key={worker.id} value={worker.id}>{worker.name} · {worker.role || 'Contractor'} · {formatStatus(worker.status)}</option>)}</select></label><div className="form-actions form-span"><button className="secondary-button" disabled={submitting || !resourceOptions.workers.length}><Users size={16} />Add crew assignment</button></div></form><form className="form-grid compact-form" onSubmit={reserveTool}><label className="form-span">Equipment<select aria-label="Equipment" required value={resourceDraft.toolId} onChange={event => setResourceDraft({ ...resourceDraft, toolId: event.target.value })}><option value="">Select registered equipment</option>{resourceOptions.tools.map(tool => <option key={tool.id} value={tool.id}>{tool.name} · {tool.category || 'Equipment'} · {formatStatus(tool.status)}</option>)}</select></label><div className="form-actions form-span"><button className="secondary-button" disabled={submitting || !resourceOptions.tools.length}><Wrench size={16} />Reserve equipment</button></div></form></div><div className="resource-ledger"><section><h3>Crew plan</h3>{selectedJob.assignments?.length ? <div className="activity-list">{selectedJob.assignments.map(assignment => <div className="activity-row" key={assignment.id}><div><strong>{assignment.workerName || 'Ledger worker'} · {assignment.role || 'Contractor'}</strong><small>{formatDate(assignment.scheduledStart)} · {formatStatus(assignment.status)}{assignment.requiresApproval ? ' · approval required' : ''}</small></div>{!['released', 'cancelled', 'completed', 'closed', 'rejected', 'declined'].includes(assignment.status) ? <button type="button" className="secondary-button" disabled={submitting} aria-label={`Release assignment for ${assignment.workerName || 'worker'}`} onClick={() => releaseAssignment(assignment.id)}><Ban size={15} />Release</button> : <span className={`status status-${assignment.status}`}>{formatStatus(assignment.status)}</span>}</div>)}</div> : <p className="workflow-note">No crew member is allocated to this job window.</p>}</section><section><h3>Equipment plan</h3>{selectedJob.tools?.length ? <div className="activity-list">{selectedJob.tools.map(reservation => <div className="activity-row" key={reservation.id}><div><strong>{reservation.toolName}</strong><small>{formatDate(reservation.neededFrom)} · {formatStatus(reservation.status)}{reservation.requiresApproval ? ' · approval required' : ''}</small></div>{!['released', 'returned', 'cancelled', 'rejected', 'declined', 'completed', 'closed'].includes(reservation.status) ? <button type="button" className="secondary-button" disabled={submitting} aria-label={`Release ${reservation.toolName}`} onClick={() => releaseToolReservation(reservation.id)}><Ban size={15} />Release</button> : <span className={`status status-${reservation.status}`}>{formatStatus(reservation.status)}</span>}</div>)}</div> : <p className="workflow-note">No equipment is reserved for this job window.</p>}</section></div></div></section></div> : null}
