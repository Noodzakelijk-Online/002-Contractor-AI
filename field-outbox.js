...
const FIELD_OPERATION_TYPES = new Set(['progress', 'production_entry', 'daily_log', 'inspection_checklist', 'observation', 'incident', 'punch_item', 'attendance_check_in', 'attendance_check_out', 'material_receipt', 'equipment_check_out', 'equipment_return']);
...
export async function enqueueFieldOperationDraft({ id, type, jobId, payload, operatorScope }) {
  ...
  if (drafts.length >= MAX_OPERATION_DRAFTS || existingSize + operationSize(draft) > MAX_TOTAL_OPERATION_BYTES) {
    throw new Error('The field operation outbox is full. Reconnect and sync existing attendance, equipment custody, daily logs, production output, progress updates, inspection checklists, material receipts, field risk reports, and punch items before saving another draft.');
  }
