...
export async function enqueueFieldOperationDraft({ id, type, jobId, payload, operatorScope }) {
  ...
  if (drafts.length >= MAX_OPERATION_DRAFTS || existingSize + operationSize(draft) > MAX_TOTAL_OPERATION_BYTES) {
    throw new Error('The field operation outbox is full. Reconnect and sync existing attendance, safety briefings, work permits, equipment custody, daily logs, production output, daywork quantities, progress updates, inspection checklists, material, expense, and environmental records, field risk reports, and punch items before saving another draft.');
  }
