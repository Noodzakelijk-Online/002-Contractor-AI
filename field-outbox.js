...
const FIELD_OPERATION_TYPES = new Set(['progress', 'daily_log', 'inspection_checklist', 'observation', 'incident', 'punch_item']);
...
export async function enqueueFieldOperationDraft({ id, type, jobId, payload, operatorScope }) {
  ...
  if (drafts.length >= MAX_OPERATION_DRAFTS || existingSize + operationSize(draft) > MAX_TOTAL_OPERATION_BYTES) {
    throw new Error('The field operation outbox is full. Reconnect and sync existing daily logs, progress updates, inspection checklists, field risk reports, and punch items before saving another draft.');
  }
